...
### Knowledge Table access control

Enterprise organizations with fine-grained access controls (FGA) enabled can restrict read access to individual Knowledge Tables at the user level. Admins use the **Share** button on each Knowledge Table page to grant or revoke read-only access for specific users. Group-based permissions are not supported for Knowledge Tables. See [Sharing a Knowledge Table](/build/knowledge/create-knowledge#sharing-a-knowledge-table) for instructions.
