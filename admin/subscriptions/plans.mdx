...
<Tabs>
  <Tab title="New billing (Actions and Vendor Credits)">
    ...
    Eval runs also consume actions: each evaluator Check (scoring rule) that runs on a conversation costs 1 action. For details, see [Billing for eval runs](/build/agents/build-your-agent/evals#billing-for-eval-runs).
