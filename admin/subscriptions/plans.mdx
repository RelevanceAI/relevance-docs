...
<Tabs>
  <Tab title="New billing (Actions and Vendor Credits)">
    ...
    - **Vendor Credits:** $10 USD per 10,000 Vendor Credits
    ...
    The Plan & Billing page includes a time-series area chart that displays your concurrency usage over time. This chart shows how many tasks are running simultaneously across your Organization.

    **What is concurrency usage?**

    Concurrency usage refers to the number of tasks executing at the same time. Each subscription tier has a specific concurrent task limit — when you reach this limit, additional tasks are queued until capacity becomes available.

    **Why monitor concurrency?**

    Monitoring your concurrency usage helps you:
    - Identify peak usage periods when you're approaching your tier's capacity
    - Understand if tasks are being queued due to capacity limits
    - Determine when to upgrade to a higher tier for increased concurrent task capacity
