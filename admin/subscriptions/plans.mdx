...
Organization managers also get an "Org total" / "By project" toggle in the chart header, which breaks concurrency down per project with a per-project tooltip. See [Analytics](/enterprise/analytics#per-project-breakdown) for details.
