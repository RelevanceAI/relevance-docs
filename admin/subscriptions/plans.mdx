...
<Tabs>
  <Tab title="New billing (Actions and Vendor Credits)">
    ...
    - On any agent's **Analytics** tab: open the agent, then click **Analytics**. The chart shows the same organization-wide concurrency data, so you can correlate that agent's activity with overall concurrency patterns.
