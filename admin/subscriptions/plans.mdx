...
<Tabs>
  <Tab title="New billing (Actions and Vendor Credits)">
    ...
    <Note>
      If you're building an agent with Inventor, the Inventor sidebar shows a real-time credit counter for credits used during that session. See [Create an Agent](/build/agents/create-an-agent) for details on the Inventor interface.
    </Note>
