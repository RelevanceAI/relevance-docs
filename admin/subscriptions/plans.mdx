...
<Tabs>
  <Tab title="New billing (Actions and Vendor Credits)">
    ...
    The Plan & Billing page includes a time-series area chart that displays your concurrency usage over time. Monitoring your concurrency helps you identify peak usage periods and determine when to upgrade for increased capacity.
    ...
    Each agent's analytics page includes the same concurrency chart, showing organization-wide concurrency data. This lets you correlate a specific agent's activity with overall concurrency patterns — useful for identifying which agents are most active during peak concurrency periods.

    To view concurrency for a specific agent:

    1. Navigate to the agent you want to investigate
