...
<Note>
  For how concurrent task limits work, including what happens when your Organization reaches capacity, see the [System Quotas](/admin/system-limits#concurrent-tasks) page.
</Note>
...
If you can manage the organization, the bars are split by project, with a per-project tooltip. Otherwise the whole organization renders as a single band. See [Analytics](/enterprise/analytics#per-project-breakdown) for details.
