...
When building tools in Relevance AI from an AI coding environment, you can configure a tool's output transformation and schema after creation — without opening the Relevance AI UI or recreating the tool. This is done using the `relevance_set_tool_output` tool available through the Relevance AI MCP server.
