...
When building tools in Relevance AI from an AI coding environment, you can configure a tool's runtime output map and output schema after creation — without opening the Relevance AI UI or recreating the tool. This is done using the `relevance_set_tool_output` tool available through the Relevance AI MCP server.

This enables workflows where tools are created and fully configured programmatically, including from Inventor agent pipelines. See [Configuring tools via MCP](/integrations/mcp/programmatic-gtm/mcp-server#configuring-tools-via-mcp) for parameters, use cases, and a worked example.
