...
Once connected, the agent automatically fetches and displays the available tools from the MCP server. Tools appear in the conversation task view and can be referenced in the agent's core instructions. If the server also exposes resources (read-only data such as documentation or schemas), the agent can read those in the background when it needs context — no extra configuration required.
...
<AccordionGroup>
  ...
  <Accordion title="What is the difference between MCP tools and MCP resources?">
    Tools perform actions (creating a record, sending a message). Resources are read-only data the agent can pull in for context (documentation, schemas). When you connect an MCP server, the agent gets access to both automatically — no extra setup for resources.
  </Accordion>
