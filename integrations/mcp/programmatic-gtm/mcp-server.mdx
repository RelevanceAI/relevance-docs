...
Reads a line-numbered window of an agent's system prompt, returning a specific range of lines rather than the full prompt. Call it multiple times with different `start_line` values to page through a large prompt section by section.

`agent_id` identifies which agent to read from. `start_line` sets the first line to return (default: 1). `num_lines` controls how many lines to read in one call (default: 50). The tool returns the requested lines with their line numbers, the total line count in the prompt, and a `has_more` flag that indicates whether additional lines follow the returned window.
...
<Accordion title="Example: read lines 100–150 of a Support Agent's prompt">
  ...
  The AI client calls `relevance_read_agent_system_prompt` with `agent_id` set to your Support Agent's ID, `start_line: 100`, and `num_lines: 50`. It returns lines 100–150 with their line numbers, plus the total line count and whether there are more lines after line 150. You can then ask to continue reading from line 151, or search for a specific term using `relevance_search_agent_system_prompt`.
</Accordion>
...
`agent_id` identifies which agent to search. `pattern` is a regex string matched against each line. `context_lines` controls how many surrounding lines to include with each match (default: 2). The tool returns each matching line with its line number plus the surrounding context lines, giving you enough orientation to understand how the term is used in each location.
...
`agent_id` identifies which agent to edit. `old_text` is the exact string to find and replace — it must match the existing text exactly, including whitespace and punctuation. `new_text` is the replacement string. The tool returns confirmation that the draft was created along with the line numbers affected by the replacement.
