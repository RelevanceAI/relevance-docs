...
The Relevance AI MCP server includes tools for managing your project's tools, not just triggering agents. `relevance_set_tool_output` lets you configure a tool's output transformation and schema after the tool has been created.
...
Configures the output transformation and schema for an existing tool.

| Parameter | Description |
|---|---|
| `tool_id` | The ID of the tool to configure |
| `output_transformation` | Maps, filters, or reshapes the tool's raw output for downstream use |
| `output_schema` | Enforces a specific structure for the tool's output |
...
<Accordion title="Typical workflow: create and configure a tool's output">
  After creating a tool in your Relevance AI project, call `relevance_set_tool_output` with the tool's ID to define how its output is shaped. Provide an `output_transformation` to map or filter the raw result, and an `output_schema` to enforce the output structure. You can call this tool repeatedly as you iterate — no recreation needed. For the full programmatic GTM context, see [Programmatic GTM](/integrations/mcp/programmatic-gtm/introduction).
</Accordion>
