...
**Member role and above** have full access to all tools, agents, knowledge, and project resources in the connected project.

**Chat role** users can connect with restricted access when the `mcp-chat-mode` feature flag is enabled for your project. Chat-role users can:
- List agents shared with them
- Trigger those agents
- Poll for run results
- List tasks
- Cancel runs

<Note>Chat-role MCP access is disabled by default and requires the `mcp-chat-mode` feature flag to be enabled on your project.</Note>
