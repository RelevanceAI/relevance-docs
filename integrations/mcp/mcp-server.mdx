...
| Tool | Description |
|------|-------------|
| `relevance_list_project_mcp_servers` | Returns the MCP servers connected to the authenticated project. Each entry carries `mcp_server_config_id`, `label` (the connection name), `url` (server endpoint), and a `status` of `disconnected` when the server is attached to an agent but no longer matches a project connection. Use this to discover what MCP servers are available before calling tools on them. |
| `relevance_list_project_mcp_server_tools` | Makes a live call to a specific MCP server and returns its currently available tools. Takes the `mcp_server_config_id` from the list above. Reflects the real-time state of the external server, so a tool missing here means the server is unreachable rather than that the tool was removed. |

### Execution tools

`relevance_trigger_agent` sends a message to an agent and `relevance_poll_agent_result` retrieves the outcome. See [handling long-running agent executions](#handling-long-running-agent-executions) for how the two work together.
