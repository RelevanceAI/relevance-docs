...
Your project role determines what you can access through the MCP server.

**Member role and above** have full access to all tools, agents, knowledge, and project resources in the connected project — the full set under [available tools](#available-tools).

**Chat role** users can connect with restricted access. Chat-role users can list the agents shared with them, trigger those agents, poll for run results, list tasks, and cancel runs. They cannot reach tools, knowledge, or the rest of the project's resources.

<Note>Chat-role MCP access is off by default and is rolling out progressively — reach out to your account manager to have it enabled for your project.</Note>
...
The Relevance AI MCP server exposes the following tools to connected clients.

### Discovery tools

| Tool | Description |
|------|-------------|
| `relevance_list_project_mcp_servers` | Returns all MCP servers connected to the authenticated project. Each entry includes an `id` (unique identifier), `label` (human-readable connection name), `url` (server endpoint), and a flag indicating if the server is disconnected. Use this to discover what MCP servers are available before calling tools on them. |
| `relevance_list_project_mcp_server_tools` | Makes a live call to a specific MCP server and returns its currently available tools. Takes a server reference (`id` from the list above) as input. Reflects real-time state of the external server for dynamic capability discovery. |

### Execution tools

| Tool | Description |
|------|-------------|
| `relevance_trigger_agent` | Triggers an agent synchronously and waits for it to complete. Has a 120-second timeout — use for agents that complete quickly. |
| `relevance_trigger_agent_async` | Triggers an agent and returns a `conversation_id` immediately without waiting for completion. Use for long-running agents. |
| `relevance_poll_agent_result` | Polls for the result of an async agent execution using the `conversation_id` returned by `relevance_trigger_agent_async`. |
...
<AccordionGroup>
  ...
  <Accordion title="MCP server listed as disconnected in relevance_list_project_mcp_servers">
    The external server may be temporarily offline — try again later. Check the server URL and credentials in the Agent builder under the **Tools** tab. If the issue persists, reconnect the server from the Agent builder to refresh the connection state.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Can I restrict which tools are available via MCP?">
    ...
    External clients can also use `relevance_list_project_mcp_servers` and `relevance_list_project_mcp_server_tools` to programmatically enumerate the MCP servers and tools available in a project. This lets developers building on top of the MCP server dynamically discover the tool surface without needing prior knowledge of the project's configuration.
  </Accordion>
</AccordionGroup>
