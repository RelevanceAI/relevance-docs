...
<Warning>
  **IMPORTANT: Required Header for GitHub API Calls**
  
  When using the GitHub API Call tool step, you **must** include the following header for the API calls to work properly:
  
  ```
  accept: application/vnd.github+json
  ```
  
  Without this header, you may encounter errors such as:
  - `'Please enter a value for 'response_body' field'`
...
- **Knowledge Integration** - Use GitHub repositories as a knowledge source for your agents, enabling them to access and reference your codebase and documentation when responding to queries.
- **Slack Integration** - Combine GitHub and Slack integrations to create powerful workflows where agents can notify team members about pull requests, issues, and deployments.
- **API Tool Step** - Learn more about making custom API calls in the [API Tool Step documentation](/tool/tool-steps/api).
