...
<CardGroup cols={2}>
  ...
  <Card title="SuperGTM" icon="bolt">
    Uses a per-user OAuth token. Each user connects their own Slack account individually, giving SuperGTM access to act on that user's behalf in Slack.
  </Card>
</CardGroup>

<Info>
If Slack works in one part of Relevance AI but not another, your workspace admin has likely approved one connection type but not the other. Ask your admin to check permissions for both the Relevance AI Slack app (for Builder) and per-user OAuth app installations (for SuperGTM).
</Info>

Both connection types can coexist in the same workspace. For details on the SuperGTM Slack connection, see [SuperGTM integrations](/get-started/chat/super-gtm/integrations).
...
<AccordionGroup>
  ...
  <Accordion title="Why does Slack work in SuperGTM but not in Builder (or vice versa)?">
    Builder and SuperGTM use different Slack authentication methods. Builder features (Agents, Tools, Triggers, Workforces) use a workspace-level bot token through the Relevance AI Slack app, which requires a workspace admin to approve the app installation. SuperGTM uses per-user OAuth, where each user connects their own Slack account individually.

    If Slack works in one area but not the other, your workspace admin has likely approved one connection type without the other. Ask your admin to check the app permission settings for both the Relevance AI Slack app (for Builder) and per-user OAuth app installations (for SuperGTM). Both connection types can coexist once both are approved. See [SuperGTM integrations](/get-started/chat/super-gtm/integrations) for details on the SuperGTM Slack connection.
  </Accordion>
</AccordionGroup>
