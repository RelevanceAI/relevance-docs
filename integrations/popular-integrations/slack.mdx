...
<Note>Both the magic link flow and the standard OAuth flow achieve the same outcome — your Slack account is linked to your Relevance AI account.</Note>
...
<CardGroup cols={2}>
  <Card title="Builder: Agents, Tools, Triggers, Workforces" icon="robot">
  ...
  <Card title="Per-user OAuth" icon="bolt">
...
## Agent-sent Slack cards

When an agent responds to a user in Slack, it can include interactive cards with action buttons. These cards appear in the thread and let users take structured actions without leaving Slack.

### Button types

Agent-sent cards support three button types:

- **message** — clicking the button sends a predefined message back into the agent conversation.
- **form** — clicking the button opens a single-page form for collecting user input, which the agent receives as a message.
- **wizard** — clicking the button opens a multi-step modal form. Users navigate through steps with Next and Back buttons, and all collected answers are delivered to the agent as a single message when they complete the final step.

You can configure button types when building an agent that uses the Send Message tool step to post cards to Slack.

### Wizard button type

The wizard button type opens a multi-step modal when a user clicks an action button on an agent-sent Slack card. Users move through steps in sequence using the Next and Back navigation buttons.

**Draft persistence.** Progress is saved automatically at each step. If a user closes the modal partway through, their answers are preserved and the modal resumes from where they left off when reopened.

**Typed confirmation.** The final step can optionally require users to type a specific phrase before they can submit — useful for confirming destructive or high-stakes actions.

**Single-message delivery.** When a user completes all steps, every answer from every step is compiled and sent to the agent conversation as one message.

<Warning>
  The wizard button type is currently available to users with early access. If this feature isn't showing on your account, [contact support](/get-started/support) to request access.
</Warning>

<Accordion title="Technical details for workspace admins">
  The wizard button type is enabled via the config flag `integration-slack_setup_wizard`, which is off by default. Contact [support](/get-started/support) to have it enabled for your account.

  Wizard drafts include optimistic concurrency controls to handle simultaneous edits correctly. A cross-region relay mechanism ensures the feature works across distributed deployments.
</Accordion>
...
When an agent sends a message to Slack, it can include interactive buttons that let recipients take action without leaving Slack. Each button has a type that determines what happens when a user clicks it.

<CardGroup cols={3}>
  <Card title="Message" icon="message">
    Clicking the button sends a predefined message back to the agent as if the user typed it. Use this for quick confirmations or simple one-click replies.
  </Card>
  <Card title="Form" icon="table-list">
    Clicking the button opens a modal in Slack with one or more input fields. The user fills in the form and submits, and the agent receives the collected data.
  </Card>
  <Card title="Wizard" icon="list-check">
    Clicking the button opens a multi-step modal in Slack. The user moves through a guided sequence of steps, completing inputs at each stage before advancing to the next. The agent receives all responses once the final step is submitted.
  </Card>
</CardGroup>

### Using the wizard button type

The wizard type is suited for workflows where collecting information in a single form would be overwhelming, or where later steps depend on earlier answers. Each step in the wizard can present different input fields, and users can navigate forward through the sequence.

<Note>
Wizard buttons are configured when building the card your agent sends. The number of steps and the fields at each step are defined at build time, not at runtime.
</Note>

To send a Slack card with a wizard button, use the **Slack: Send Message (Advanced)** tool step and include a button with type set to `wizard`. Define each step as an object with a title and its list of input fields. When the user submits the final step, the wizard closes and the responses are delivered to the agent.

<AccordionGroup>
  <Accordion title="Example: multi-step intake wizard">
    A support agent sends a card asking a user to submit a request. The wizard walks them through three steps:

    1. **Request type** — a dropdown asking whether the request is a bug report, feature request, or general question.
    2. **Details** — a text area for a description and an optional file attachment field.
    3. **Priority and contact** — a priority selector and an email field.

    The agent receives all three steps' data as a single payload once the user submits step 3.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Magic link expired or not working">
  ...
  <Accordion title="Consent page shows wrong email address">
...
<AccordionGroup>
  ...
  <Accordion title="What is the magic link I received from Slack?">
  ...
  <Accordion title="Do I need to use the magic link or can I connect directly from Relevance AI?">
  ...
  <Accordion title="What happens if I'm not logged in when I click the magic link?">
  ...
  <Accordion title="Why does Slack work in one part of Relevance AI but not another?">
...
<Frame caption="See how to manage your entire AI workforce directly inside Slack - from triggering workforces to monitoring live task status, routing outputs, and processing files.">
  <iframe
    ...
    src="https://www.youtube.com/embed/9U6hDy1re9Q"
    title="How to Manage Your AI Agents Directly in Slack"
    allowFullScreen
  ></iframe>
</Frame>
