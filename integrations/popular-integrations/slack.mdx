...
## Collecting structured input with Slack Forms

Slack Forms are native Slack modals that let agents collect structured input from users mid-task. Instead of parsing a free-form message, the agent displays a form directly in Slack with specific fields the user must fill in before the task continues. Forms support both text input fields and dropdown menus (single-choice select), and can be organized into multi-step wizard flows. See the [Slack Forms](/integrations/popular-integrations/slack-forms) page for full configuration details.
...
<AccordionGroup>
  ...
  <Accordion title="Can my agent show a dropdown or form to collect user input in Slack?">
    Yes. Agents can display Slack Forms — native Slack modals — to collect structured input mid-task. Forms support text input fields and dropdown menus, so users can type a response or pick from a predefined list. See the [Slack Forms](/integrations/popular-integrations/slack-forms) page for setup and configuration details.
  </Accordion>
