...
## View Configured Agents in a Channel

You can quickly see which agents are configured in any Slack channel by using the `/agents` slash command. This displays a private list of all agents available in that channel, along with their trigger keywords and quick access to their settings.

### How to use

Simply type `/agents` in any Slack channel where Relevance AI agents are configured.

### What you'll see

When you run the `/agents` command, you'll receive an ephemeral message (visible only to you) that shows:

- All agents configured for that channel
- The trigger keywords for each agent
- Links to view or edit each agent's configuration

<Tip>
  The `/agents` command is useful when you need to:
  
  - Discover which agents are available in a channel
  - Check what keywords trigger specific agents
  - Quickly access agent settings without leaving Slack
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="How do I see which agents are configured in my Slack channel?">
    Use the `/agents` slash command in any Slack channel where Relevance AI agents are configured. This will show you a private list of all agents available in that channel, their trigger keywords, and links to view or edit their settings.
  </Accordion>
