...
Twilio does not currently support event-based triggers in Relevance AI. To react to inbound SMS or calls, set up a [webhook trigger](/build/agents/build-your-agent/triggers/webhook) on your agent and configure your Twilio phone number's webhook URL to point to it.
...
<AccordionGroup>
  ...
  <Accordion title="Can I receive inbound SMS or calls?" icon="inbox">
    Inbound messages and calls are handled via webhooks. Configure your Twilio phone number's inbound webhook URL to point to a [webhook trigger](/build/agents/build-your-agent/triggers/webhook) on your Relevance AI agent. Your agent will then be activated each time a message or call arrives.
  </Accordion>
