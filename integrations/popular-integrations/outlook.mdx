...
<Warning>
  **Attachment Size Limit**: The Outlook trigger has a **25MB per-attachment limit** due to Microsoft Graph API constraints. Emails with attachments larger than 25MB will not be processed by the trigger.
  
  **Workarounds**:
  - Compress large files before sending (keep attachments under 25MB)
  - Convert large DOCX files to PDF format (often reduces file size significantly)
  - Use cloud storage links (OneDrive, SharePoint) instead of large attachments
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Why aren't emails with large attachments triggering my agent?">
    The Outlook trigger has a **25MB per-attachment limit** due to Microsoft Graph API constraints. Emails with attachments larger than 25MB will not be processed by the trigger.
    
    **Solutions**:
    - Ask senders to compress large files before sending (keep attachments under 25MB)
    - Convert large DOCX files to PDF format, which often reduces file size significantly
    - Use cloud storage links (OneDrive, SharePoint, Dropbox) instead of attaching large files directly
    - Set up a separate workflow to handle large file transfers outside of email
    
    This is a Microsoft Graph API limitation, not a Relevance AI limitation.
  </Accordion>
