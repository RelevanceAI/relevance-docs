...
The Thread Reply Method controls whether your agent posts a response back to the Teams thread after processing a message. Two options are available:

- **Agent auto-reply** (default) — the agent processes the message and posts its response back to the Teams thread.
- **No reply** — the agent processes the message silently, without posting to Teams.

The "No reply" option is useful when the agent's output goes somewhere other than Teams — for example, writing results to a database, sending an email, or triggering a downstream workflow. This setting is per-trigger and opt-in; the default maintains backward compatibility.

To configure, open your MS Teams trigger settings in Relevance AI, expand **Advanced Settings**, and select your preferred option under **Thread Reply Method**.
