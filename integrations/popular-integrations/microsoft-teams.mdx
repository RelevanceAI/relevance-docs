...
<Note>Both the magic link flow and the standard OAuth flow achieve the same outcome — your Microsoft Teams account is linked to your Relevance AI account.</Note>
...
<Frame caption="Selecting the Thread Reply Method in a Microsoft Teams trigger's advanced settings.">
  ![Thread Reply Method setting in Microsoft Teams trigger advanced settings](/images/teams-thread-reply-method.png)
</Frame>
...
<Steps>
  ...
  <Step title="Choose a reply method">
...
<Steps>
  <Step title="Open your trigger settings">
    Go to your agent's **Microsoft Teams** trigger in Relevance AI.
  </Step>
...
The calendar trigger lets agents respond automatically when calendar events occur in the connected Microsoft or Teams calendar. Instead of triggering on incoming messages, the agent fires based on scheduled events — allowing it to run tasks before a meeting starts, send follow-ups after an event, or take action whenever a calendar entry is created or updated.

#### Advanced settings

##### Event sensitivity

The **Event sensitivity** setting controls which calendar events activate the trigger, based on how the event is marked in Outlook or Teams.

<CardGroup cols={2}>
  <Card title="Normal" icon="calendar">
    Regular calendar events with standard visibility.
  </Card>
  <Card title="Private" icon="lock">
    Events marked as Private in Outlook or Teams, which hide details from others in the calendar view.
  </Card>
</CardGroup>

Both options are selected by default, so the agent triggers on all calendar events regardless of sensitivity. You can deselect either option to filter which events activate the agent:

- Deselect **Private** to prevent private calendar events from triggering agent tasks.
- Deselect **Normal** to trigger the agent only on private events.

This is useful when you want to keep personal or private meetings from activating automated workflows.

<Note>Existing calendar trigger setups continue to work unchanged — Event sensitivity is an optional filter added for new and existing configurations.</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Can my agent process Teams messages without replying?">
  ...
  <Accordion title="What is the magic link I received from Microsoft Teams?">
