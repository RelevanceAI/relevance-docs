...
Evaluation alarms can send notifications to Microsoft Teams channels when your Agent's evaluation metrics breach configured thresholds. This lets you monitor Agent quality automatically — when a pass rate drops below your threshold and stays there for the configured evaluation period, a notification is posted to your chosen Teams channel.

To receive evaluation alarm notifications in Teams, set up a Monitor dashboard in your Agent's Evaluate tab, then add an alarm and select Microsoft Teams as a notification channel. Connect your Microsoft account and choose the team and channel where alerts should be posted. See [evaluation alarms](/build/agents/build-your-agent/evals#alarms) for full configuration details.
