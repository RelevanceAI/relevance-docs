...
Authentication uses HTTP Basic auth with the API key as the username and the API secret as the password.

### Cluster details

| Field | Description |
|-------|-------------|
| **REST Endpoint URL** | The per-cluster REST Proxy endpoint (not the control plane URL). Find this under **Cluster Overview → Cluster Settings → Endpoints** in your Confluent Cloud console. It follows the format `https://<cluster-id>.<region>.confluent.cloud:443`. |
...
### Generic Confluent Cloud API call

Make any request to the Confluent Cloud Kafka REST API. This tool step automatically substitutes the `cluster_id` into API paths, so you can use path templates like `/kafka/v3/clusters/{cluster_id}/topics` without manually inserting the ID.

Use this for operations not covered by the purpose-built steps below.
...
Deletes a topic from the cluster by name. This action is irreversible.
...
- **Event streaming automation**: Trigger agent actions in response to Kafka topic data, or produce records that drive downstream systems.
- **Topic management**: Automate topic creation and cleanup as part of environment provisioning workflows.
- **Real-time data ingestion**: Use agents to produce enriched or transformed records to Kafka topics from external data sources.
- **Monitoring consumer lag**: Regularly list consumer groups and alert when groups fall behind, enabling proactive incident response.
