...
User Level Authentication enables agents to operate using each user's individual authentication accounts for integrations, allowing multiple users to safely and privately use the same shared agent (especially in Chat) while only accessing their own data. Builders can configure this behavior per tool input, and users only need to authenticate once per integration.
...
As a builder, you can configure User Level Authentication at the tool input level. If you want to enable User Level Authentication for a tool step that currently has authentication configured directly on the step, you'll need to convert that authentication to a tool input instead.

<div style={{width: "100%", position: "relative", "padding-top": "56.75%"}}>
  <iframe 
    src="PLACEHOLDER-SETUP"
    frameBorder="0"
    title="Enable user level authentication at tool input level"
...
2. Navigate to the OAuth Input you want to enable User Level Authentication on and click the authentication settings icon
...
<Tip>
If you have authentication configured directly on a tool step and want to enable User Level Authentication, you'll need to convert that authentication to a tool input field. This allows each user to provide their own credentials when using the tool.
</Tip>
...
<div style={{width: "100%", position: "relative", "padding-top": "56.75%"}}>
  <iframe 
    src="PLACEHOLDER-CHAT"
    frameBorder="0"
    title="Using user level authentication in chat"
    allow="clipboard-write; fullscreen"
    webkitAllowFullscreen="true"
    mozAllowFullscreen="true"
    allowFullscreen
...
<AccordionGroup>
  ...
  <Accordion title="Can I use both shared accounts and individual accounts in the same project?">
    Yes, if you're on an Enterprise subscription with asset-level authentication controls enabled. When User Level Authentication is configured for a tool input, Enterprise users will see both their individual accounts and any shared project-level accounts available for selection. This gives you flexibility to choose the appropriate account based on the context. Non-Enterprise users will only see their individual accounts.
  </Accordion>
  
  <Accordion title="What integrations support User Level Authentication?">
    User Level Authentication works with all integrations that require authentication in Relevance AI. This includes popular integrations like Google Drive, Gmail, Slack, HubSpot, Salesforce, and many others. The feature is integration-agnostic and will work with any tool that uses an authenticated connection.
  </Accordion>
