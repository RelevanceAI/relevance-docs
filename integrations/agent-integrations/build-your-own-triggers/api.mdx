...
<Note>
  When you trigger an agent via API, it starts a task but does not return the result directly. Instead, you must configure where the output goes. For example, a Google Sheet or knowledge table. The agent will complete its task and send the result to your specified destination, not back through the API response.
</Note>
