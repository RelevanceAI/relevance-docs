...
<Note>
  When you trigger an agent via API, it starts a task but does not return the result directly. Instead, you must configure where the output goes — e.g., a Google Sheet or Knowledge Table. The agent will complete its task and send the result to your specified destination, not back through the API response.
</Note>
...
3. Look for the "API trigger" section, locate the "Customer Triggers" area
