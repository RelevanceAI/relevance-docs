...
You can also switch projects in the Builder using the command palette. Press `Cmd+K` (Mac) or `Ctrl+K` (Windows/Linux) to open it, then type a project name to search across all your projects, including those in other regions. Select a result to switch immediately.

If you're currently viewing a page that exists in the destination project (for example, an Agent page), the Builder will try to land you on the same page type after switching. For more details, see [Command palette](/get-started/builder/command-palette).
