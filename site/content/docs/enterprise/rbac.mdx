...
| Run a chat (LLM)                       | ✅         | ✅          | ✅          | ✅          | ✅        |
| Authenticate MCP                       | ✅         | ✅          | ✅          | ✅          | ✅        |
| MCP access mode                        | Full       | Full        | Full or run-only | Run-only | Chat mode |
...
<AccordionGroup>
  ...
  <Accordion title="MCP access">
    Chat role users can authenticate the Relevance AI MCP server but are automatically placed into Chat mode — a restricted subset of tools limited to read access and running agents. Write and destructive tools are not available in the session. Dynamic (user-level) OAuth connections are not supported for Chat role users; if an agent requires dynamic authentication, assign the user a Member role in a dedicated project instead.
  </Accordion>
</AccordionGroup>
