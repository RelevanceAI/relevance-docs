...
<Note>
  Email is used as the unique account identifier in Relevance AI. If a user's email address changes in your identity provider, Relevance AI provisions a new account rather than updating the existing one — permissions, group memberships, and owned assets are not transferred. See [Email address updates for IdP-managed users](/enterprise/rbac-groups#email-address-updates-for-idp-managed-users) for details and the recommended workaround.
</Note>
