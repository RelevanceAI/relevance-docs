...
<Tabs>
  ...
  <Accordion title="Invent Workbench" icon="browser">
    When you open an agent, tool, or knowledge table from the Invent panel, that page renders in a tabbed interface called the Workbench — the main navigation sidebar is hidden while the Workbench is active.

    Each resource you open becomes a new tab showing that resource's icon and title. Tabs accumulate as you navigate, letting you switch between them without losing your place.

    <Steps>
      <Step title="Switch between tabs">
        Click any tab to bring that page into focus.
      </Step>
      <Step title="Close a tab">
        Click the close button on the tab to remove it from the Workbench.
      </Step>
      <Step title="Restore normal navigation">
        Close Invent entirely to exit the Workbench and return to the standard navigation sidebar.
      </Step>
    </Steps>

    Your tab session persists across page reloads within the same project.

    <Note>
      Invent Workbench is part of the IA v2 experience and is currently in early access, available to specific Relevance staff only.
    </Note>
  </Accordion>
    </AccordionGroup>
  </Tab>
  ...
  <Accordion title="Scheduled check-ins" icon="calendar-clock">
    ...
    <Note>
      Before setting up a scheduled check-in, add an email or Slack notification channel in [Alerts](/build/invent/invent#alerts-accordion-title) so Invent can notify you about the result.
    </Note>
...
<Tabs>
  ...
  <Tab title="Configure an alarm">
    ...
    Turn on **Auto-diagnosis by Invent** to investigate the first breach and send a diagnosis and proposed fix to the alarm creator's [Invent alert channels](/build/invent/invent#alerts-accordion-title). Invent still shares its findings if the cause is unclear and never applies changes without your approval.
