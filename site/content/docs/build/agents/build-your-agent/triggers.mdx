...
![Trigger configuration in the agent builder](/images/agent/triggers1.png)
...
<Info>
  New to Relevance AI? [Relevance AI Academy](https://relevanceai.com/academy) has structured courses that walk you through building on the platform step by step.
</Info>
...
The Triggers homepage gives you a project-level view of all triggers across your agents. Navigate to **Triggers** in the main left navigation — outside of any specific agent — to see the full list. Each row shows the trigger name, the agent that owns it, its type, and current status.

### Run trend chart

<Info>
The run trend chart and trigger details drawer are in early access and available to beta users.
</Info>

A stacked bar chart at the top of the Triggers homepage shows trigger activity across your entire project over the last 7 days. Green segments represent completed runs; red segments represent failed runs.

### Trigger details drawer

Click any trigger row to open a details drawer. The drawer shows:

- **Name, owner, and type** — identifies the trigger and which agent it belongs to
- **Status** — whether the trigger is active or paused
- **Last successful run** — timestamp of the most recent completed run
- **Last failed run** — timestamp of the most recent failed run
- **7-day run trend** — a stacked bar chart of completed (green) vs failed (red) runs for that specific trigger
