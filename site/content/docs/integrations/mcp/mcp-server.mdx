...
The MCP server exposes tools for agents, tools, workforces, knowledge, and evals. `relevance_run_evaluation` is the tool for running eval batches — it supports both scenario-based and historical scoring modes, where you pass existing `task_ids` and `check_ids` to grade already-completed production conversations without re-executing them. See [agent skills](/integrations/mcp/agent-skills) for the full runbook on using evals via MCP.
