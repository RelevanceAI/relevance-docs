 ...
 */
// content/_snippets is a symlink to the repo's _snippets/, and the .mdx files
// are reached through symlinks too, so file.path is a REAL path. Resolve the
// snippet root to its real location or the relative include never matches.
const SNIPPET_ROOT = fs.realpathSync(
  path.resolve(import.meta.dirname, '../content/_snippets'),
);
