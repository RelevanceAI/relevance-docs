...
<Tabs>
    <Tab title="contains">
        List rows where entries in the field `description` contains the term `Durian BID`:
        ```js
            {
                "field": "description",
    ...
    <Tab title="word_match">
    List rows where entries in the field `description` matches the words `Home curtain`:
    ```js
        {
            "field": "description",
    ...
    List rows where entries in the field `brand` contain `LG` or `samsung`:
    ```js
        {
            "field": "brand",
