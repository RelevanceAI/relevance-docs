...
<Tip>
LLM outputs are always string. So even if they resemble a list, a JSON object, a number or a boolean, make
sure to convert the output from string to the actual type when necessary.
</Tip>

For non-programmers readers, let us look at an example to understand the difference easier:
A string is just a chain of characters with absolutely zero underlying structure. Consider this text, 
