...
- **Delete tool** (Tools section): Removes the tool from your project and all agents. Deleted tools can be restored via Inventor.
- **Delete a tool step** (Agent Builder → Tools): Removes a single action within an existing tool's workflow. This cannot be undone.

# Delete a tool

When you no longer need a tool, you can delete it from your project. If you change your mind, deleted tools can be restored via Inventor.
...
- The tool can be restored via Inventor if needed

## Restore a deleted tool

Deleted tools can be restored via Inventor.

1. Open Inventor from the sidebar
2. Ask Inventor to restore the tool, for example: "Restore the [tool name] tool"
