...
For supported OpenAI, Google, and Anthropic models, you can configure how much the model reasons before responding. Reasoning uses output tokens to work through complex problems, which improves quality on hard tasks but is slower and costs more.
...
Reasoning settings are per-model, so the available options depend on the model and provider you select, and are ignored if they don't match:
...
Some Claude models use adaptive thinking and adjust their reasoning depth automatically, with effort levels up to high and max. You can also influence reasoning through the system prompt. See [thinking type and effort level](/build/agents/build-your-agent/agent-settings/language-model#thinking-type-and-effort-level) for the Claude models that support it.

#### Cost impact

<Warning>
Reasoning and thinking tokens count as output tokens, not a separate category, so enabling thinking increases the cost of your LLM Tool step. Monitor usage when you turn it on.
</Warning>
