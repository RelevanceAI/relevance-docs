...
Reasoning configuration only applies when the selected model supports it. The setting appears in the step configuration for that model. Reasoning is supported across current thinking-capable models from all three providers:
...
<Note>
Because support is per-model, the options that appear can change when you switch models. If model and vendor options are mismatched, or the model does not support configurable reasoning, these settings are ignored.
</Note>
