...
To loop over the output of a previous step, enable variable mode on the **List to loop** field and use `{{step-name.output-field}}` syntax. For example, if an earlier step named `python` returns a list in a field called `transformed`, you would enter `{{python.transformed}}` as the list source.
...
4. Run the Tool. The loop produces one output tab per item in the list, each containing the LLM's response for that movie.
...
When a loop runs, each item in the list produces a separate output tab in the results panel. If your list has five items, you will see five output tabs — one per iteration. This lets you inspect the result for each individual item rather than a single aggregated output.

If an item errors during processing, its tab appears under the **Errors** tab of the loop step results by default (see error handling settings below).
...
<AccordionGroup>
  <Accordion title="Can I set up a loop in a loop?">
    Yes. You can add loops within loops if your workflow requires it. This allows you to create more complex processes tailored to your specific needs. Keep in mind that nesting increases complexity, so make sure it aligns with your overall flow.
  </Accordion>
