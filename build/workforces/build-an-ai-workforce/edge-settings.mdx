...
## Parallel Tool Calls compatibility

When an agent has [Parallel Tool Calls](/build/agents/build-your-agent/agent-settings/language-model#parallel-tool-calls-beta) enabled, all connections originating from that agent must use **Create new task** as their Task Behavior. Parallel handoffs require independent task instances that can execute simultaneously — "Continue same task" forces sequential execution and is incompatible with this mode.

Connections that conflict with this requirement are highlighted in yellow in the Workforce Builder, signaling that you need to update the Task Behavior setting.
...
Repeat for each conflicting connection on agents that have Parallel Tool Calls enabled.
