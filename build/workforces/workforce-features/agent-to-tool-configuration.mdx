...
Each Tool listed in the agent's **Tools** section also has **Edit tool**, which opens it in the Tool builder — useful for small fixes without leaving the Workforce. Editing a Tool used by several agents changes it for all of them, so clone it first if you only want the change here. See [Editing shared tools](/build/tools/create-a-tool#editing-shared-tools).
...
- [Trigger to Agent Configuration](/build/workforces/workforce-features/trigger-to-agent-configuration): Learn how to configure the initial triggers that activate your agents
- [Tool to Tool Configuration](/build/workforces/workforce-features/tool-to-tool-configuration): Discover how to connect tools directly to other tools for automated workflows
- [Condition to Tool Configuration](/build/workforces/workforce-features/condition-to-tool-configuration): Explore how to add conditional logic to your tool executions
