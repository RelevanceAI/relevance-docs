...
When many tasks are waiting on the same tool, the Approvals tab groups them by the pending tool and owning agent so you can triage at scale rather than reviewing each task individually. See [Task Operations](/build/agents/give-your-agent-tasks/tasks-page) for bulk actions on these groups.
