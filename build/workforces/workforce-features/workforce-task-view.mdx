...
Click the expand chevron on any agent row to open a nested sub-table of that agent's individual tasks. The sub-table shows up to 50 tasks at a time; if more match the current filters, a **Show more** button loads the next page. Clicking any task row opens the full task detail drawer, where you can review conversation history, tool usage, outputs, and errors.
