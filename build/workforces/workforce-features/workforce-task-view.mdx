...
The Task View supports grouping tasks **By Agent**, giving you one summary row per agent in your workforce. To see an agent's individual tasks, click the expand arrow on that agent's row. A nested table opens showing up to 25 of that agent's tasks. Click **Show more** to load additional tasks using cursor-based pagination.

Clicking any task row in the expanded list opens the task detail drawer. All active filters and search terms you've applied to the main view carry through to the nested task list, so the tasks shown always reflect your current filter context.
