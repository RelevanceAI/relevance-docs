...
When a parent agent spawns a child agent, a live activity card appears inline in the timeline showing the child agent's real-time status, step count, and current tool activity. You can expand the card to view the full conversation history for that child agent run, including tool outputs and any error messages. This makes it straightforward to trace exactly what each agent in your workforce did during a task.
