...
<Tabs>
  ...
  <Accordion title="Review and annotate a plan" icon="comment-lines">
    When Invent generates a plan, you can select specific passages and leave inline comments, or add an overall note, before sending all feedback to Invent in one structured message.

    #### Inline comments

    Highlight any passage of text in the generated plan to open the comment input. Type a comment and confirm it — the comment becomes anchored to that specific passage. Anchored comments remain attached to their referenced text even if the plan is re-rendered or edited. Add multiple inline comments across different passages before sending.

    #### Overall note

    Use the overall note field for general remarks about the document as a whole — not tied to any specific passage.

    #### Sending feedback

    Click **Send feedback** to deliver all inline comments and the overall note to Invent as a single, structured message. Invent receives each comment with the exact passage it references, so it can precisely locate and address each piece of feedback. After sending, your feedback appears in the conversation history as formatted comment cards so you can review what was submitted.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Can I leave comments on a plan Invent has created?">
    Yes. Select any passage in a generated plan to add an inline comment anchored to that text. Use the overall note field for general remarks. Click **Send feedback** to deliver everything to Invent at once. Your comments remain attached to the correct passages even if the plan is later edited.
  </Accordion>
</AccordionGroup>
