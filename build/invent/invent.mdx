...
<Tabs>
  ...
  <Accordion title="Sounds" icon="volume-high">
    The Sounds preference in Invent Settings → General lets you configure audio notifications for when Invent turns end. This helps you stay informed about important events without constantly watching the screen. The preference is saved per-device and does not sync across devices.
    ...
    <Steps>
      ...
      <Step title="Open General">
        Select **General**, choose your preferred sound setting under **Sounds**, then click **Save**.
      </Step>
    </Steps>
    ...
    | **Needs attention** | Plays sounds for approvals, questions, and errors. |
    | **All messages** | Plays sounds for all Invent responses. |

    <Note>
      Four distinct sound types are used: attention (for pending approvals and questions), error, build (when builds complete), and message (for plain replies). The preference is stored in localStorage per-device and does not sync across devices. Only one tab plays audio at a time — the active viewing tab stays silent. Sounds requires the `inventor-turn-sounds-fe` feature flag.
    </Note>
  </Accordion>
