...
<Tabs>
  ...
  <Accordion title="Routines" icon="repeat">
    ...
    Routines are recurring instructions that Invent runs on a schedule — daily, weekly, monthly, or annually. You or Invent can create a routine directly from a conversation when a recurring task is appropriate.

    Each routine runs as its creator, using their connected accounts and permissions. Only the creator can edit or re-arm a routine. Other team members can pause or delete a routine as an emergency stop.

    Because routines run without access to previous conversation context, write instructions to be self-contained and understandable on a cold read.

    #### Create a routine

    Tell Invent what you want done on a recurring basis and how often. Invent will suggest creating a routine when it identifies the task as recurring. Use the time-of-day picker in the schedule modal to set the exact hour, minute, and AM/PM.

    #### Manage routines

    Open the schedule management modal to view, edit, or delete routines and check-ins from a single list.
  </Accordion>
