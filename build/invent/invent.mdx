...
<Tabs>
  ...
  <Accordion title="Check-ins" icon="calendar-clock">
    ...
    A check-in is a one-time scheduled wake-up that resumes the current Invent conversation at a specific time. Use check-ins for one-time follow-ups — for example, to review results after a deployment or revisit a task once a process completes.

    You or Invent can schedule a check-in directly from a conversation. For recurring work, use routines instead.
