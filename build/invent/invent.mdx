...
Its job continues after launch. Invent reviews live performance, detects quality problems, and proposes and tests improvements. You bring the process knowledge; Invent builds and manages the Agent team underneath it.
...
<Tabs>
  <Tab title="Session approvals">
    <img src="/images/invent-ask-first.png" alt="The Ask first and Auto-accept options in the Invent message composer" />
    ...
    <Note>
      Ask first and Auto-accept control tool approvals during execution. Plan first mode is a separate, independent setting that requires explicit sign-off on a written plan before any execution begins — the two controls serve different purposes.
    </Note>
  </Tab>

  <Tab title="Tool permissions">
    <img src="/images/invent-permissions.png" alt="Invent Permissions settings showing approval controls for individual tools" />
    ...
    | **Ask** | Requests approval unless **Auto-accept** is on for the session. |
    | **Always ask** | Requests approval every time, even when **Auto-accept** is on. |
  ...
  <Tab title="Plan first">
    Enable Plan first from the **+** menu in the Invent composer. The toggle persists across navigation until you turn it off.

    When Plan first is on, Invent follows this sequence before building anything:

    1. **Interview** — Invent asks questions to understand your problem and the outcome you need.
    2. **Feasibility check** — Invent checks what can be built given your project's current setup.
    3. **Written plan** — Invent drafts a plan as prose in the transcript. An expand button opens it full-screen for easier reading. The plan content is surfaced directly in the transcript rather than buried in collapsed timeline blocks.
    4. **Sign-off** — Invent pauses and presents an **Approve** button. Nothing is built until you approve.

    To request changes before approving, reply in the chat. The old **Deny** button has been replaced with a "Want it changed? Reply below" hint.

    Subagents and evaluation runs skip the plan sign-off step automatically.

    <Note>
      Plan first requires the InventorPlanMode flag to be enabled on your account. The toggle only appears in the + menu if your backend supports it.
    </Note>
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Accordion title="Start a task" icon="wand-magic-sparkles">
    ...
    The **+** menu beside the composer also contains the [Plan first](/build/invent/invent#approval-controls) toggle, which adds a structured interview and sign-off step before Invent builds anything.
  </Accordion>
  <Accordion title="Plan first" icon="clipboard-list">
    Plan first mode is an opt-in workflow that adds a structured planning phase before Invent makes any changes. Invent interviews you, checks feasibility, and writes a plan — then waits for your explicit approval before building anything. Use it for complex tasks where you want to review the approach before any changes are made.

    <Note>
      Plan first mode is only available to organizations where Relevance AI has enabled it. Contact your account team to request access.
    </Note>

    #### How to enable

    Before starting a new conversation, click the **+** button in the Invent composer to open the options menu, then toggle **Plan first** on.

    #### Workflow

    <Steps>
      <Step title="Interview">
        Invent interviews you to understand the problem, systems involved, judgment calls required, and definition of success.
      </Step>
      <Step title="Feasibility check">
        Invent checks what the current project setup and connected integrations can support.
      </Step>
      <Step title="Plan presentation">
        Invent writes a plan and presents it inline in the conversation, with an **Expand** button for full-screen viewing.
      </Step>
      <Step title="Sign-off">
        An **Approve** button appears. Invent will not begin building until you click it, even if Auto-accept is on for the session.
      </Step>
      <Step title="Revision">
        To request changes, reply in the chat or click **Reject**. Invent incorporates your feedback and presents an updated plan.
      </Step>
    </Steps>

    <Note>
      Subagent tasks and evaluation runs bypass the plan sign-off step automatically. These are platform-initiated operations, not new build requests, so the planning phase does not apply to them.
    </Note>
  </Accordion>
  ...
  <Accordion title="Memories" icon="brain">
    ...
    When Memory is enabled, Invent can save and recall details you want it to remember across conversations, such as naming conventions, preferred models, workflows, and recurring constraints.
...
<AccordionGroup>
  ...
  <Accordion title="Does Invent publish changes automatically?">
    By default, Invent makes changes in draft so you can review them before they go live. Auto-accept lets Invent apply changes without pausing for each approval, but restoring an earlier version always requires confirmation. With [Plan first mode](/build/invent/invent#plan-first) enabled, Invent also requires you to approve a written plan before any changes are attempted.
  </Accordion>

  <Accordion title="What is Plan first mode?">
    Plan first mode is an opt-in workflow where Invent interviews you, checks feasibility, and writes a plan for your explicit approval before making any changes — regardless of your Auto-accept setting. It is designed for complex tasks where you want to review the approach before any execution begins. For setup instructions and the full workflow, see [Plan first](/build/invent/invent#plan-first) in the Invent features section.
  </Accordion>
  ...
  <Accordion title="What does Plan first mode do?">
    Plan first mode adds a structured review step before Invent builds anything. When enabled, Invent interviews you to understand your problem, checks what can be built, and drafts a written plan in the transcript for you to read and approve. Nothing is built until you click **Approve**. To request changes, reply in the chat. Enable it from the **+** menu in the composer; the toggle persists until you turn it off.
  </Accordion>
</AccordionGroup>
