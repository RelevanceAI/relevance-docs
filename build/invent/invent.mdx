...
Invent is always available to organization admins and owners, regardless of any project-level settings.

For all other team members, Invent access is controlled per project. A project admin can enable or disable Invent from **Settings → Organization → Projects → Manage settings → Features**. The setting defaults to disabled, so Invent must be explicitly enabled for each project where you want team members to access it.

<Note>
  When Invent is disabled for a project, all Invent runs are blocked at the API level and all Invent entry points are hidden for team members — including the sidebar navigation, floating button, chat panel, and evaluation CTAs.
</Note>

To configure per-project Invent access, see [Project features](/admin/project-management/project-features).
...
<AccordionGroup>
  ...
  <Accordion title="Who can use Invent?">
    Invent is always available to organization admins and owners. For all other team members, access is controlled per project. A project admin can enable Invent from **Settings → Organization → Projects → Manage settings → Features**. The toggle defaults to disabled — Invent must be explicitly enabled for each project where you want team members to use it.
  </Accordion>
