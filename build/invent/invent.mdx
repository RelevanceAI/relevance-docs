...
<Tabs>
  ...
  <Tab title="Web search">
    <Note>
      Web search is currently behind feature flags and is not yet available to all users.
    </Note>
    ...
    A globe icon in the message composer lets you enable or disable web access on a per-message basis. It's opt-in — toggle the globe on before sending a message to give Inventor web access for that request. The toggle state persists across session remounts, so you don't need to re-enable it each time you open Inventor.

    ### What Inventor can do with web access on

    When web access is enabled, Inventor can search the web in real time (powered by Exa), fetch and read the content of web pages, and access current information beyond its training data. This is useful for researching competitors, checking documentation, or pulling in recent data while you're building.

    ### URL paste prompt

    If you paste a URL into the message composer while web access is disabled, a banner appears prompting you to enable it so Inventor can fetch that page.
