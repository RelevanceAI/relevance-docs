...
Knowledge uses retrieval-augmented generation (RAG): the agent searches your knowledge base at runtime and retrieves the most relevant chunks for each query. This makes it well suited for large or frequently updated document sets where you only ever need a portion of the content at once.

[Context](/build/agents/build-your-agent/context) works differently — it injects the full file into the agent's context on every run. Use Context for small, essential reference files that the agent always needs in full. Use Knowledge for larger datasets where selective retrieval is more practical.
...
- [Context](/build/agents/build-your-agent/context): Attach files directly to your agent's context window without setting up a knowledge base.