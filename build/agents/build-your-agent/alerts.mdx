...
<CardGroup cols={3}>
  ...
  <Card title="Trigger failure" icon="bell-exclamation">
    Monitors when a specific trigger fails. Per-user subscription — each team member controls their own alerts independently.
  </Card>
...
Monitors when a specific trigger fails. Subscriptions are per-user — each team member manages their own alarms independently, so one person enabling or disabling an alarm has no effect on others. There is one alarm per trigger per user.

Set up trigger failure alarms from the Triggers monitoring page using the alarm bell icon next to each trigger. The icon shows three states: on, off, or unavailable.

When you create or re-enable an alarm, the watermark is set to the current time. Only failures that occur after that point will trigger a notification — failures that happened before the alarm was active are not reported.
