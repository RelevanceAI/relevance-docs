...
6. Add at least one notification channel (Email, Slack, or Microsoft Teams)
7. Click **Save**
...
Triggers when a tool fails during execution. You can optionally scope this to specific tools — leave the selection empty to watch all tools, or pick individual tools to monitor.

This is useful for keeping an eye on critical integrations where a tool failure could block your agent's workflow.

#### Agent published

Triggers when the agent is published. No additional configuration is needed — simply select a notification channel and you'll be alerted whenever a new version of the agent goes live.
...
Add one or more email addresses. No OAuth connection is required — just enter the addresses and notifications will be sent directly.

#### Slack

Connect your Slack workspace via OAuth, then select a channel to receive notifications. See [Slack integration](/integrations/popular-integrations/slack) for setup details.
...
When your agent encounters a situation beyond its capabilities, it can escalate the conversation to a human manager using the built-in **Escalate to Manager** tool.
