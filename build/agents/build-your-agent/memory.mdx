...
Three MCP tools are available for working with memory snapshots. Unlike the built-in tools described above, these are accessed programmatically by any client connected to the [MCP Server](/integrations/mcp/programmatic-gtm/mcp-server).
...
The get-agent API response includes a `latest_memory_snapshot_id` field. This field contains the ID of the most recently created snapshot for that agent, or `null` if no snapshots exist.
