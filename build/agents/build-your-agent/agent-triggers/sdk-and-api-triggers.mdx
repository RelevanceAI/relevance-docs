...
The SDK is the recommended way to trigger your Agent programmatically. It's a higher-level interface for JavaScript and TypeScript — no need to manage raw HTTP requests or authentication headers manually.
...
The SDK ships to both npm and JSR and works in Node.js, Deno, Bun, Cloudflare Workers, and the browser.
...
For the full SDK reference — installation across every runtime, embed keys for browser apps, workforces, streaming, and more — see the [JavaScript SDK docs](/sdk/introduction).
...
<AccordionGroup>
  <Accordion title="What programming languages can I use?">
    The SDK is available for JavaScript and TypeScript. The API trigger works with any language capable of making HTTP requests.
  </Accordion>
