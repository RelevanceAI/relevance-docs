...
5. Fill in the scenario details:
    ...
    | **Approval handling** | How the eval handles tool approval requests — prevents the eval from hanging when your agent or workforce has approval-gated tools. See [Approval handling](#approval-handling) for details on each mode. | auto |
...
8. (Optional) Set the **Approval handling** mode to control how the eval handles approval-gated tools. If your agent or workforce has tools that require human approval before executing, eval runs would otherwise hang indefinitely while waiting for a human who never arrives.

    | Mode | Behavior |
    |------|----------|
    | **Auto** (default) | The simulated user decides each approval request intelligently based on the scenario context |
    | **Approve all** | Every approval request is automatically confirmed; the tool executes |
    | **Reject all** | Every approval request is automatically denied; the tool does not execute |

    When an approval is granted, the tool actually executes during the eval — the run tests end-to-end behavior rather than stopping at the approval gate. Use **Approve all** when testing what your agent does after a tool runs, **Reject all** when testing how your agent handles a denied tool, and **Auto** for contextual decisions that reflect realistic user behavior.

9. Click **Save test scenario** to save your configuration.
...
When an agent or workforce uses tools that require human approval before running, a running eval would pause and wait indefinitely for a human to respond — causing the eval to hang. The **Approval handling** dropdown in the scenario configuration form lets the eval resolve those requests automatically.

| Mode | Behavior |
|------|----------|
| **auto** | A simulated AI evaluates each approval request based on conversation context and decides whether to approve or deny it. This is the default. |
| **approve_all** | Every approval request is automatically confirmed. The approved tools execute and their real output becomes part of the conversation. |
| **reject_all** | Every approval request is automatically denied. The agent receives a rejection and continues from there. |

Use **auto** when you want realistic approval decisions that vary with context — the simulated AI may approve some requests and deny others based on what makes sense in the conversation. Use **approve_all** when you need to test the full tool execution path end-to-end, such as verifying that downstream steps behave correctly after an approval. Use **reject_all** when you want to verify that your agent handles denied requests gracefully — for example, that it informs the user, escalates, or falls back to an alternative.

Approval handling works for both agent and workforce scenarios.
...
<AccordionGroup>
  ...
  <Accordion title="My agent uses approval-gated tools — will evals hang waiting for approval?">
    No. Use the **Approval handling** dropdown in the scenario configuration to control what happens when the eval reaches an approval gate. Set it to **Approve all** to automatically confirm every approval request (the tool will execute), **Reject all** to automatically deny every request, or leave it on **Auto** to let the simulated user decide based on context. Without this setting configured for approval-gated tools, an eval run would pause indefinitely waiting for a human who never arrives.
  </Accordion>
  ...
  <Accordion title="What happens when my agent has approval-gated tools?">
    If your agent or workforce uses a tool that requires human approval, an eval scenario would pause indefinitely waiting for a response. The **Approval handling** setting in the scenario configuration prevents this by automatically resolving approval requests. Set it to **auto** to let a simulated AI decide each request contextually, **approve_all** to confirm every request so the tools actually execute, or **reject_all** to deny every request and test how your agent handles rejection. See [Approval handling](#approval-handling) for details.
  </Accordion>
</AccordionGroup>
