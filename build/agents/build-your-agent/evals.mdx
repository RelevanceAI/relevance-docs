...
<Note>
Evals also run against entire workforces, not just individual agents. Workforce evals are currently API-only — drive them via the [Relevance AI MCP](/get-started/core-concepts/programmatic-gtm) or the eval API directly. Native UI access is on the way.
</Note>

The Evals section is your command center for testing and evaluating AI Agent performance. Located in the **Evaluate** tab (next to the Build and Use tabs) in the Agent builder, Evals lets you create test sets, define reusable checks, run automated evaluations, and track live Agent quality — all without manual testing.

![Evaluate tab showing the Evals sidebar (Test, Runs, Checks, Publish, Performance) and a Performance dashboard with overall score, total runs, and checks breakdown](/images/agent/agent-evals.png)
...
<CardGroup cols={3}>
  ...
  <Card title="Reuse checks" icon="scale-balanced">
    Define evaluation criteria once in the Checks tab and attach them to scenarios, Performance dashboards, or ad-hoc evaluations of completed tasks.
  </Card>
  <Card title="Track live quality" icon="chart-line">
    Create Performance dashboards that score live Agent tasks against your checks, with sample-rate controls and per-check trend charts over time.
  </Card>
</CardGroup>
...
- **Checks** — The reusable set of evaluation criteria. Create a check once, then attach it to scenarios, to Performance dashboards, or to one-off evaluations of completed tasks.
...
- **Performance** — Track live Agent quality on real tasks. Create one or more Performance dashboards, each with its own checks, sample rate, and conversation filters.
...
- **To a Performance dashboard** — the check runs on a sampled portion of live Agent tasks.
...
6. Attach checks to define how this scenario is scored. You can either pick existing checks from the Checks tab or create new ones inline:
    ...
    Newly created checks land in the Checks tab and can be reused on other scenarios or Performance dashboards.
...
The **Performance** section continuously scores live Agent tasks using checks you define. Unlike Test, which runs simulated conversations, Performance evaluates the real conversations your Agent is having.

Performance is organized into dashboards — you can create as many as you need for different aspects of Agent quality, different conversation types, or different teams. Each dashboard is configured independently with its own set of checks, sample rate, and conversation filters.

### Viewing your dashboards

When you open **Performance** from the left sidebar, you see a list of all dashboards for this Agent. Each card shows a preview of the dashboard's key metrics: overall score, total runs, and which checks are active. Click a card to open the dashboard's detail view.

### Creating a Performance dashboard

1. Go to the **Evaluate** tab and select **Performance** from the left sidebar.
2. Click **+ New dashboard** and give it a name.
3. A settings drawer opens on the right. Configure the dashboard:

   | Setting | Description |
   |---------|-------------|
   | **Checks** | Select which checks from the Checks tab to track on this dashboard |
   | **Sample rate** | The percentage of incoming tasks to evaluate (e.g., 50% evaluates one in every two tasks) |
   | **Conversation filter** | Optional. Limit evaluations to tasks with specific statuses (e.g., completed, escalated). Leave blank to evaluate all tasks. |

4. Click **Save**.

Once saved, qualifying tasks are automatically scored at the sample rate you set.

### Individual dashboard view

Each dashboard has its own detail page showing:

| Section | Description |
|---------|-------------|
| **Overall score** | Aggregate score across all evaluated tasks in the selected date range |
| **Total runs** | Number of tasks evaluated |
| **Overall score timeseries** | Chart showing how the aggregate score changes over time |
| **Per-check charts** | Individual trend charts for each check attached to this dashboard |
| **Version markers** | Markers aligned with Agent publishes so you can correlate score changes with specific versions |
| **Run history table** | All evaluation runs with score, task name, and a link to drill into the full conversation |

### Updating dashboard settings

To change a dashboard's checks, sample rate, or conversation filter after initial setup:

1. Open the dashboard you want to update.
2. Click **Settings** in the top-right corner of the dashboard.
3. The settings drawer opens — make your changes.
4. Click **Save**.

### Managing multiple dashboards

You can create multiple Performance dashboards for a single Agent. Each dashboard tracks independently — scores, run counts, and charts are separate per dashboard.

<Tip>
To delete a dashboard, open it, click **Settings**, then select **Delete dashboard**.
</Tip>
...
Multiple dashboards let you segment Agent quality by what matters to each team or use case without mixing signals in a single view.

<CardGroup cols={2}>
  <Card title="Track different quality dimensions" icon="chart-bar">
    Create one dashboard for tone and empathy checks, and a separate dashboard for tool accuracy and escalation handling. Each team sees only the checks they own.
  </Card>
  <Card title="Separate by conversation type" icon="filter">
    Set a conversation filter to score completed conversations on one dashboard and escalated conversations on another. Different conversation outcomes often need different quality criteria.
  </Card>
  <Card title="Manage credit spend by priority" icon="coins">
    Run high-priority dashboards at 100% sample rate for maximum signal. Use a lower rate (10–25%) on lower-priority dashboards to keep credit spend proportional to importance.
  </Card>
  <Card title="Isolate post-publish regressions" icon="code-branch">
    Create a dashboard focused on your most critical checks. After publishing a new Agent version, the version markers on this dashboard make it easy to spot regressions introduced by the update.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Set up Performance dashboards" icon="chart-line">
    Create at least one Performance dashboard with your most important checks so you catch regressions on real conversations, not just simulated ones.
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="Where do my checks live?">
    All checks live in the **Checks** tab under Evaluate. From there you can attach them to scenarios (for evaluation runs), to Performance dashboards (for live tasks), or to one-off evaluations of completed tasks. The **Scenarios**, **Dashboard**, and **Unused** filters show where each check is currently attached.
  </Accordion>
  ...
  <Accordion title="How many Performance dashboards can I create?">
    You can create as many dashboards as you need for a single Agent. Each dashboard has independent configuration, metrics tracking, and run history.
  </Accordion>

  <Accordion title="Can I use the same check on multiple Performance dashboards?">
    Yes. Checks are defined once in the Checks tab and can be attached to any number of Performance dashboards, scenarios, or one-off evaluations. Editing a check in the Checks tab updates it everywhere it's attached.
  </Accordion>
