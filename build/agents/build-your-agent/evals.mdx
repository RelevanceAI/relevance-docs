...
5. Fill in the scenario details:
    ...
    | **+ Set exact first message** | Optional — enter the exact text to send as the first turn to the Agent. The message is sent word-for-word; the simulated user does not generate a message for that turn. | "Hi, I need help with my bill." |

    <Note>
    A first message counts as one of the turns in the maximum message limit. Scenarios without a first message continue to work exactly as before — the simulated user generates all messages, including the opening one.
    </Note>
...
<AccordionGroup>
  ...
  <Accordion title="What does setting an exact first message do?">
    When you enter a first message on a scenario, that exact text is sent to the Agent as the opening turn of the conversation — the simulated user is bypassed for that first turn only. Subsequent turns continue to be generated by the simulator based on your scenario description. The first message counts toward the scenario's maximum message limit.

    This is useful when you want a specific, reproducible opening message for consistent testing — for example, to verify how the Agent handles a particular phrase or to ensure the same conversation starter across every run of the scenario.
  </Accordion>
