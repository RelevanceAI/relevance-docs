...
- **Runs** — Past evaluation run results. Browse scores, status, cost, and creation date for every run. The table supports sorting, column resizing, and a column visibility picker to show or hide optional cost breakdown columns.
...
<Note>
The table supports column sorting (click any column header to sort server-side, works across pages), column resizing (drag the edge of any header), and a column visibility picker to toggle optional columns. Column visibility choices are saved per view — batch detail and monitor views remember their settings independently. The Test column is pinned and stays visible when scrolling horizontally.
</Note>
...
| **Scenario credits** | Credits consumed by the scenario runner (user simulation LLM calls). Toggle on via the column visibility picker. |
| **Execution credits** | Credits consumed by the agent's own execution. Toggle on via the column visibility picker. |
| **Checks credits** | Credits consumed by Check evaluations. Toggle on via the column visibility picker. |
| **Checks actions** | Actions consumed by Check evaluations. Toggle on via the column visibility picker. |
...
- **A runs list** showing each evaluated task — columns described below.

<Note>
The runs list supports column sorting (click any header), column resizing (drag header edges), and a column visibility picker for optional cost breakdown columns. Visibility choices in this view are saved separately from the batch detail view. The Name column is pinned and stays visible when scrolling horizontally.
</Note>
...
| **Scenario credits** | Credits consumed by the scenario runner. Toggle on via the column visibility picker. |
| **Execution credits** | Credits consumed by the agent's own execution. Toggle on via the column visibility picker. |
| **Checks credits** | Credits consumed by Check evaluations. Toggle on via the column visibility picker. |
| **Checks actions** | Actions consumed by Check evaluations. Toggle on via the column visibility picker. |

Click any row to drill into the full evaluated conversation.
...
For a quick per-component view without leaving the table, use the column visibility picker to enable the **Scenario credits**, **Execution credits**, **Checks credits**, and **Checks actions** columns — each shows one component's cost inline.

For the full breakdown, click any **Credits** or **Actions** value to open a modal that separates the user simulation, the agent's own run, and the Checks that scored the conversation, so you can see which part drives the cost — usually the agent's own execution. The breakdown is split across three components:
