...
Each run and scenario has a **Costs** breakdown showing two totals — credits and actions — split across three components:

| Component | What it covers |
|-----------|----------------|
| **Scenario Runner** | The LLM calls used to simulate the user. Uses credits only. |
| **Agent Execution** | The cost of running the agent for the test. Uses credits, plus one action for each tool the agent runs during the conversation. |
| **Checks** | Each Check that scores the conversation, broken down by model. Each Check uses credits and counts as one action. |

The evaluation run itself isn't charged a separate action — actions come from the agent's tool runs and from each Check.

For how credits and actions work across your plan, see [Plans and credits](/admin/subscriptions/plans).
