...
5. (Optional) Set a **Task status filter** to control which task outcomes trigger evaluations.
    ...
    - **Execution limit reached** — the agent hit its step or time limit
    - **Errored** — the task ended due to an error
    ...
    <Note>
    Monitor dashboards also work for workforce evaluations. When monitoring a workforce, evaluations trigger on every state transition rather than only when a task reaches a final status. Available status options are tailored to the resource type, so workforce dashboards may show different statuses than agent dashboards.
    </Note>
