...
| Tool | Description |
|------|-------------|
| `relevance_get_pre_publish_status` | View the current gate configuration and check history for an Agent |
| `relevance_upsert_pre_publish_config` | Create or replace a gate with 1–10 eval test sets and per-test-set pass thresholds |
| `relevance_delete_pre_publish_config` | Remove the gate from an Agent |
| `relevance_poll_pre_publish_check` | Wait for an in-progress check to complete and return the result |
