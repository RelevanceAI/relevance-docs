...
The Evals section is your command center for testing and evaluating AI performance. Located in the **Evaluate** tab in the Agent builder and on the Workforce canvas, Evals lets you create test sets, define reusable Checks, run automated evaluations, and monitor live quality — all without manual testing. Evals apply to both individual Agents and Workforces, including the sub-agents and tools inside a Workforce.
...
<AccordionGroup>
  ...
  <Accordion title="Tool Usage" icon="screwdriver-wrench">
    ...
    | **Node** | Optional. When evaluating a Workforce, scope the Check to a specific graph node (Agent or Tool). If the same Agent appears more than once in the graph, each occurrence is listed separately with a connection label. Leave blank to check across the whole conversation. |

    #### Argument conditions

    You can add conditions that verify the specific arguments passed to a tool call, not just whether the tool was called. Each condition specifies an argument path and a match type, and every condition must be satisfied for the Check to pass.
    ...
    | **Contains** | The argument value must include the specified substring. Text only. |
    ...
    The available match types depend on the argument's type: **Contains** applies to text arguments, while **Exact match** is bound to the argument's own type or enum values. Conditions whose argument path can't be resolved against the tool's schema are dropped.

    <Info>
    Argument conditions work for both single-Agent evals and node-scoped Workforce evals.
    </Info>
...
7. (Optional) Add **Tool simulations** to emulate Tool usage without actually calling the underlying Tools. Tool simulations are configured per scenario:
    ...
    When evaluating a Workforce, Tool simulations are configured per node rather than globally. Each Agent and Tool node in the graph appears as a separate entry, so you can assign different simulated responses to the same Tool depending on which node calls it. If the same Agent appears in more than one position in the graph, each occurrence is listed separately with a connection label — for example, "Research Agent ← Coordinator" — so you can configure each instance independently.
...
2. Select which version to evaluate from the **Version** dropdown. It defaults to the current active version; select any historical or draft version to test against instead. This applies to both Agents and Workforces.
...
<Note>
For Workforces, the selected version pins the graph topology. Nested Agents inside the Workforce still run their own latest version.
</Note>

You can also start a run programmatically with the `relevance_run_evaluation` MCP tool — see [Running evaluations](/integrations/mcp/mcp-server#running-evaluations) on the MCP server page.
...
<Note>
The results table supports column sorting (click any column header — sorting is applied server-side, so it works across pages) and column resizing (drag the edge of any header). A column visibility picker lets you show or hide optional columns, and your choices are saved per view: the batch detail and monitor views remember their settings independently. The **Test** column is pinned and stays visible when scrolling horizontally.
</Note>
...
| **Test** | The scenario name — pinned as the first column and always visible |
...
| **Actions** | Actions consumed for this scenario — each Check plus the agent's tool runs. Only appears on usage-based plans |
...
- **A runs list** showing each evaluated task, with the columns below.

| Column | Description |
|--------|-------------|
| **Name** | The evaluated task's name — pinned as the first column and always visible |
| **Status** | Completed or Failed |
| **Score** | Percentage of Checks that passed |
| **Checks** | Pass/fail count for this run |
| **Credits** | Credits consumed for this run |
| **Actions** | Actions consumed for this run. Only appears on usage-based plans |
| **Run time** | How long the evaluation took to complete |

Click any row to drill into the full evaluated conversation. Like the scenario results table, the runs list supports column sorting, column resizing, and a column visibility picker — and its visibility choices are saved separately from the batch detail view.
...
An alarm watches the scores on a Monitor dashboard and sends a notification when the overall score drops below a threshold you set. Alarms are configured per dashboard, so you can catch regressions as they happen.
...
5. Configure the alarm settings (see below).
6. Click **Save**.
...
Alarms fire on the initial breach only. Additional breaches during the cooldown period do not trigger new notifications, and no notification is sent when scores recover.

### Auto-diagnosis by Inventor

When the **Auto-diagnosis by Inventor** toggle is enabled on an alarm, Inventor automatically investigates the breach the moment the alarm first fires and delivers a diagnosis to the configured alert channels.

The diagnosis identifies which Checks degraded, which tasks contributed to the drop, and likely causes — giving you a starting point without manually reviewing the Monitor dashboard.
...
Auto-diagnosis runs on the initial alarm trigger only. It does not run for cooldown re-notifications or on score recovery.

<Info>
  Auto-diagnosis requires at least one alert channel on your **Inventor notification settings** (Inventor settings modal → Alerts tab), not on the alarm itself — the toggle stays hidden until that is set up. Once enabled, diagnoses are delivered to the channels of whoever turned the toggle on. Auto-diagnosis depends on [Inventor](/build/invent/invent), which is a pilot for Enterprise plans, so it may not be available on your account yet.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Can I rerun a previous evaluation?">
    Yes, you can run the same scenarios again at any time. Each run is saved in your Runs history, allowing you to compare results across different Agent versions. You can also target a specific historical or draft version using the **Version** dropdown in the run modal, so you can compare versions directly.
  </Accordion>
