...
- **Runs** — Past evaluation run results. Browse average scores, tasks evaluated, progress status, credit spend, and creation date for every run. On new billing, an **Actions** column shows total action consumption per run.
...
## Billing for eval runs

Each evaluator Check (scoring rule) that runs on a conversation during an eval costs 1 action. This applies to every Check that scores a conversation across all scenarios in a run.

| Cost component | Charge |
|----------------|--------|
| Each evaluator Check per conversation | 1 action |
| Per-run platform fee | 0 (currently waived) |

### Cost breakdown modal

All customers can open a cost breakdown modal for any eval run by clicking through from the Runs list. The modal itemizes scenario runner costs, evaluator check costs, and the platform fee.

### Actions column

On new billing (Actions and Vendor Credits), the Runs table includes an **Actions** column showing total action consumption per eval run.

### Credit-billed customers

On legacy credit-based billing, eval run fees are automatically converted to credits and deducted from your credit balance.

<Note>
Action usage for Workforce eval runs is not yet itemized in the cost breakdown modal.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="How are eval runs billed?">
    Billing depends on your plan type.

    On new billing (Actions and Vendor Credits), each evaluator Check that runs on a conversation costs 1 action. The per-run platform fee is currently waived. You can see total action consumption in the **Actions** column of the Runs table, or open the cost breakdown modal for a full itemization.

    On legacy credit-based billing, credits consumed for each scenario are calculated by adding together the Agent task run, the simulator (the LLM that generates user messages), and every Check that runs on the conversation. Eval run fees are automatically converted to credits and deducted from your balance.
  </Accordion>
