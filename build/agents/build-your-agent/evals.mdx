...
<AccordionGroup>
  ...
  <Accordion title="Custom Tool" icon="code">
    Points an eval rule at a tool you've built for fully programmable evaluation logic. The eval serializes the conversation and sends it to your tool as a job; your tool returns a pass/fail verdict and an optional reason.

    <Info>
    Custom Tool checks are currently in early access. If you don't see this check type in your account, reach out to your account manager.
    </Info>

    **Tool contract requirements**

    Your evaluator tool must meet the following contract or the check will not run:

    - A [`conversation` input](/build/tools/customize-tool/inputs) — the eval passes the serialized conversation here.
    - A boolean `passed` [output](/build/tools/advanced-setup/output-configuration) — `true` means the check passed, `false` means it failed.
    - An optional `reason` string output — if present, the value is shown in the check verdict alongside the pass/fail result.

    Click **Create evaluator tool** in the check configuration panel to scaffold a new tool that already meets these requirements.

    | Field | Description |
    |-------|-------------|
    | **Tool** | Select your evaluator tool — only tools that meet the contract are shown |
    | **Reason output** | Optional — select the output field to use as the human-readable verdict explanation |

    If the selected tool is deleted or no longer meets the contract requirements, a warning badge appears on the check and it will fail until the issue is resolved.
  </Accordion>
</AccordionGroup>
...
2. Click **+ New Check**.
3. Select a **Type** (LLM Judge, Text Includes, Text Equals, Tool Usage, or Custom Tool).
...
The **Publish** section lets you choose which test sets must pass before your Agent can be published. If the results don't meet your minimum pass rate, publishing can be blocked. Gates can be configured through the Evaluate tab in the builder, or managed programmatically via Inventor or MCP tools.
...
### Managing pre-publish gates via Inventor or MCP

<Info>
Programmatic gate management is rolling out progressively. If you don't see it in your account yet, reach out to your account manager to discuss access.
</Info>

When a pre-publish gate is configured, publishing your Agent via Inventor or an MCP client automatically runs the gate check before proceeding. If the check passes, the Agent is published immediately. If it fails, publishing is blocked and the check result is returned.

Four MCP tools are available for managing pre-publish gates programmatically:

| Tool | Description |
|------|-------------|
| `get_pre_publish_status` | View the current gate configuration and check history for an Agent |
| `upsert_pre_publish_config` | Create or replace a gate with 1–10 eval test sets and per-test-set pass thresholds |
| `delete_pre_publish_config` | Remove the gate from an Agent |
| `poll_pre_publish_check` | Wait for an in-progress check to complete and return the result |

To bypass a configured gate when publishing via Inventor or MCP, pass the `skip_checks` parameter. Use this only when you have a deliberate reason to skip evaluation — for example, an urgent hotfix where you have already validated the change.

<Warning>
If the gate configuration cannot be read at publish time, the publish is blocked. This fail-closed behavior prevents accidental publishes when gate config is unavailable.
</Warning>
...
<Note>
Custom Tool checks run as a job and are billed based on what the tool does internally. If your evaluator tool calls an external model or service via a [BYO API key](/get-started/core-concepts/api-integration), those vendor costs are not charged as Relevance AI Vendor Credits — you pay the external provider directly, not twice. The 1 Action per Check still applies regardless of check type.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="What is a Custom Tool check and how do I build a compliant evaluator tool?">
    A Custom Tool check lets you point an eval rule at a tool you've built, giving you fully programmable evaluation logic. When the check runs, the eval serializes the conversation and sends it to your tool as a job; your tool returns a verdict.

    To be eligible as an evaluator tool, it must have a [`conversation` input](/build/tools/customize-tool/inputs) and return a boolean `passed` [output](/build/tools/advanced-setup/output-configuration). You can also add an optional `reason` string output — if present, its value is shown alongside the pass/fail result in the check verdict.

    The easiest way to get started is to click **Create evaluator tool** in the check configuration panel, which scaffolds a new tool that already meets the contract requirements. Only tools that satisfy the contract appear in the tool picker when configuring a Custom Tool check.
