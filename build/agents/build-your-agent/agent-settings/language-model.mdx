...
Reasoning configuration only applies when the selected model supports it. Mismatched settings are ignored.
...
<Note>
Standard Claude Sonnet (non-thinking variants) does not support reasoning configuration. These settings are ignored if you select a non-supported model.
</Note>
