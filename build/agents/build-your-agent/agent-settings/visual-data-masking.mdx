...
<Warning>
All four PII types are automatically masked when the feature is enabled. You cannot choose which types to mask or customize the masking format. This is a simple toggle feature with no advanced configuration options.
</Warning>
...
Visual Data Masking and PII Redaction are complementary features that protect sensitive information at different stages. They serve different purposes and work together to provide comprehensive privacy protection.
...
- Visual Data Masking protects what your team sees in the platform
- PII Redaction (Enterprise) protects what gets exported to your S3 bucket
...
<Tabs>
  ...
  <Tab title="Combining with other features">
    ...
    - **Visual Data Masking** protects UI visibility during collaboration
    - **PII Redaction** protects exported data for compliance
    - **User Level Authentication** protects data access at the integration level
    - **RBAC** protects who can access agents and projects
    - **Data Retention** protects by removing old data entirely

    Together, these features provide comprehensive privacy protection across the entire data lifecycle.
  </Tab>
  ...
  <Tab title="Testing and validation">
    ...
    - Communicate to your team that this is a Beta feature with known limitations
    - Explain that AI detection is not 100% reliable
    - Ensure team members understand this is visual only and doesn't modify backend data
    - Document any PII types that are consistently missed for your use case
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="AI detection can miss PII">
  ...
  <Accordion title="International format detection">
    Detection accuracy is optimized for US/standard formats. International phone numbers, non-Latin character names, and region-specific data formats may not be detected reliably.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="What if Visual Data Masking misses PII in my conversations?">
    AI detection can occasionally miss PII, especially with non-standard formats. If you notice consistent detection failures for specific data types, document these cases and contact support. Do not rely solely on Visual Data Masking for critical privacy requirements. Consider implementing additional privacy controls at the data source or using PII Redaction (Enterprise) for backend protection.
  </Accordion>
</AccordionGroup>
