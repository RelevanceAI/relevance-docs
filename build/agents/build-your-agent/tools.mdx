...
## Using tools as custom evaluators

Any tool you build can be used as an evaluator in the Evals system. When a tool accepts a `conversation` input and returns a `passed` boolean output, it meets the evaluator contract and will appear in the Custom Tool check picker. This lets you apply external API calls, complex business rules, or deterministic algorithms as evaluation criteria. See [Custom Tool checks](/build/agents/build-your-agent/evals#check-types) for details.