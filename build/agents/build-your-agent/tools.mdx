...
## Editing and creating tools from the Agent Builder

You can open any tool attached to your agent directly from the Agent Builder — no need to navigate to the Tools sidebar separately. There are three ways to open a tool:

- Click the **Open editor** button in the tool header
- Open the overflow menu (⋮) next to the tool and select **Open editor**
- Click the tool row

All three open the canonical Tool Builder in the current tab — the same full editor available under **Tools** in the left sidebar. Any unsaved changes to your Agent are saved automatically before navigation.

Edits you make in the Tool Builder affect the tool's draft globally. If the tool is shared across multiple agents, those changes will apply to all of them. See [Editing shared tools](/build/tools/create-a-tool#editing-shared-tools) for details on how to avoid affecting other agents.

### Creating a new tool from the Agent Builder

You can create a new tool without leaving the Agent Builder. From the **Tools** section, use the option to add a new tool and select to create one from scratch. The Tool Builder opens in the current tab. When you save the new tool, it is automatically attached back to the agent you started from.
