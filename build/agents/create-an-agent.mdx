...
Instead of typing, you can describe your agent in a live voice conversation with Inventor. Click **Talk to Inventor** to open the full-screen voice overlay.

<Note>
Voice interview mode is an experimental feature available to select users. LLM and speech costs apply during voice sessions and are charged to your account.
</Note>

The **Talk to Inventor** button appears in two places:

- As a card on the Inventor start screen before you've started a conversation
- As a microphone icon in the message composer toolbar during an active conversation

The voice overlay shows a connection status indicator, a live transcript with speaker labels, and visual indicators for Inventor's current state — listening, thinking, or speaking.

Describe your agent conversationally — what it should do, how it should behave, and what integrations it needs. Inventor will ask clarifying questions to gather what it needs.

When you're done, choose one of two options:

- **End conversation** — closes the voice session without building an agent
- **End & build** — closes the session and triggers Inventor to build the agent based on the conversation
