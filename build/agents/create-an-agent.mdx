...
After creating or editing an agent in Inventor or the agent builder, your changes save as a draft. Nothing is live until you explicitly publish.

This applies to all changes: the initial agent creation, prompt and configuration edits, and attaching or modifying tools. The interface shows a visual indicator for whether you're viewing the **Draft** or **Live** version of your agent.

To make your draft live:

1. Click **Publish** in the agent builder
2. Confirm in the dialog that appears

Both steps are required — even with auto-approve enabled, publishing always requires explicit confirmation. When you test inside Inventor or the agent builder, the test runs against the draft version, so you can verify your changes before they reach users.

For a full breakdown of draft management, version history, and per-tool editing, see [Draft and publish](/build/agents/publish-your-agent).
