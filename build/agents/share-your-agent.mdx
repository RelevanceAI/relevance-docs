...
<Warning>
  **User Level Authentication is not supported in embedded chat widgets.** If your agent uses [User Level Authentication](/enterprise/user-level-authentication) (an Enterprise feature that allows each user to authenticate with their own credentials), this will not work in embedded chat widgets. For embedded agents, you'll need to configure your agent to use a shared account for integrations instead.
</Warning>
...
Embed Relevance AI Chat as a tab within Microsoft Teams, allowing your team members to interact with your agent directly inside Teams. This deployment option is designed specifically for the Teams environment and handles authentication differently from standard web embeds.

### How it works

When you add Relevance AI Chat as a Teams tab, users can access your agent without leaving the Teams interface. Because Teams tabs cannot redirect users away from the Teams context, the chat uses a popup-based authentication flow instead of the standard redirect-based flow.

When a user needs to authenticate:
1. A popup window opens for login
2. The user completes authentication in the popup
3. The popup closes automatically
4. The user continues their conversation in Teams

This approach keeps users within Teams while still providing secure authentication.

### Setting up Chat as a Teams tab

<Steps>
  <Step title="Get your Chat URL">
    From your agent's Share menu, copy the Chat UI URL or iframe embed code.
  </Step>
  
  <Step title="Add a tab in Teams">
    In your Teams channel or chat, click the **+** button to add a new tab.
  </Step>
  
  <Step title="Choose Website">
    Select **Website** from the tab options.
  </Step>
  
  <Step title="Configure the tab">
    - Enter a name for your tab (e.g., "AI Assistant")
    - Paste your Relevance AI Chat URL
    - Click **Save**
  </Step>
</Steps>
...
<Info>
  The popup authentication flow is automatic when Chat detects it's running inside a Teams tab. No additional configuration is required.
</Info>

If your agent uses integrations that require user authentication, users will see a popup window when they first interact with features that need authentication. This is different from standard web embeds where authentication happens via redirect.

### Limitations

<Warning>
  **User Level Authentication is not supported in Teams tabs.** Similar to other embedded scenarios, if your agent uses [User Level Authentication](/enterprise/user-level-authentication), this will not work when embedded as a Teams tab. Configure your agent to use a shared account for integrations instead.
</Warning>

- Popup blockers may interfere with authentication. Users should allow popups from Teams.
- Some browser extensions may affect the popup authentication flow.

### Teams integration vs. Teams tab

This Teams tab deployment is different from the [Microsoft Teams integration](/integrations/popular-integrations/microsoft-teams), which allows agents to monitor Teams channels and respond to messages automatically. 

- **Teams tab** (this section): Embeds Chat UI for direct user interaction within Teams
- **Teams integration**: Connects agents to monitor and respond to channel messages as a trigger

You can use both approaches together — for example, an agent that monitors a support channel and is also available as a tab for direct questions.
...
<AccordionGroup>
  <Accordion title="Why can't I clone an agent that I originally cloned from the Marketplace?">
    ...
    If you need another copy of the agent, you can return to the [Marketplace](/get-started/marketplace/introduction) and clone the original listing again or add it to other projects directly from there.
  </Accordion>
  ...
  <Accordion title="Why can't I use Copy to Project on an agent I cloned from the Marketplace?">
    ...
    To add the agent to additional projects, visit the [Marketplace](/get-started/marketplace/introduction) and clone the original listing directly to each project where you need it.
  </Accordion>
</AccordionGroup>
