...
## Sharing labels in list views

The agent list displays visual indicator pills next to agent names so you can see which sharing options are active without opening each agent's settings.

- **Embeddable** — an embed option (Chat UI, Chat Widget, or Task UI) is enabled. Anyone with the link can run the agent using your credits.
- **Cloneable** — the Clonable Template option is enabled. Anyone with the link can clone the agent into their own workspace.
