...
Agents and Workforces are non-deterministic — the same input can produce a different answer, and a change to a prompt, a Tool, or a model can shift behavior somewhere you weren't looking. You write Tests that simulate the users and situations you care about, and Evals scores each one against success criteria you define — so when a score moves, it's the behavior that changed, not the way you tested.
