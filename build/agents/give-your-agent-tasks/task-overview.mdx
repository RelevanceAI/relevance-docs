...
Tasks can be grouped **By Agent** to see an agent-level summary in the task monitoring interface. When this grouping is active, each row in the list represents one Agent.

To drill into an Agent's individual Tasks, click the expand arrow on that Agent's row. A nested table opens showing up to 25 of that Agent's Tasks. If there are more, click **Show more** to load the next page using cursor-based pagination.

- Clicking any Task row in the expanded list opens the task detail drawer.
- All active filters and search terms apply to the nested task list, so the Tasks shown reflect your current filter context.
