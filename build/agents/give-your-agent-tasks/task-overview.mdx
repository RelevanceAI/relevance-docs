...
- **Errors** - Tasks that failed or encountered errors, with a 7-day timeline chart
...
The **Errors** tab shows all tasks that failed or encountered errors. At the top of the tab, an interactive timeline chart displays error counts for each of the last 7 days.

- Click a bar in the chart to filter the task list to errors from that specific day. The selected bar highlights in red to indicate the active filter.
- To clear the date filter, click the filter indicator that appears above the task list, or use the standard filter controls.

This chart helps you spot error spikes quickly — for example, if a tool integration broke on a specific date, you can click that day's bar to review all affected tasks at once.
