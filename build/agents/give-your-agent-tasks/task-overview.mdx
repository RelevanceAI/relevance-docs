...
The **Credits** and **Actions** chips in the sidebar show total spend for the task, including any evaluation costs incurred during live monitoring. Hover over either chip to see a tooltip that explains what is included in the total.

Clicking the **Credits** chip opens the cost breakdown modal for the task. The modal shows:

- Total credits and actions consumed
- A **by source** table splitting task run spend from evaluation costs
- Collapsible detail sections for inspecting each cost source

<Note>
The cost breakdown modal here shows costs for a single task run. See the [Analytics](/enterprise/analytics) page for aggregate cost data across all tasks for an agent or workforce.
</Note>
