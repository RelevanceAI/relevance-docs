...
- **Approvals** — Tasks waiting on your approval before they can continue. Tasks are grouped by the tool requiring approval and the agent that owns the task, making it easier to triage many pending requests at once.
...
Within the Approvals tab, tasks are organized into groups based on which tool is waiting for approval and which agent owns the task. Each group shows:

- The name of the tool requiring approval
- The name of the agent that triggered it
- How many tasks are in that state

Use the pagination controls to move through groups when there are many pending approvals. Select a group to see the individual tasks it contains, then open any task to review and act on it.
