...
<Accordion title="What is the difference between the Google Sheets clear row and Delete rows tool step?">
    When you use [Delete Rows](/tool/tool-steps/google-sheets/delete-rows), the remaining rows shift up to fill the gap. When you use Clear row, the rows are cleared in place (the data is removed, but the rows themselves remain the same).
</Accordion>
</AccordionGroup>
