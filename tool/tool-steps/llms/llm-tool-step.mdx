...
<AccordionGroup>
  ...
  <Accordion title="Too large data">
    When using Relevance to handle large inputs by selecting the most relevant entries, if the input data is too large, you need to upload it as a dataset use it as a [knowledge](/knowledge/introduction) in your Tool. Maximum size for non-knowledge data is 131,072 tokens (~90kb).
    ```
