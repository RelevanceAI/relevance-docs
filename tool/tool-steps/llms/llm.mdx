...
* [Conversation history](/tool/tool-steps/llms/llm-tool-step#conversation-history)
* [System prompt](/tool/tool-steps/llms/llm-tool-step#system-prompt)
* [Temperature](/tool/tool-steps/llms/llm-tool-step#temperature)
* [Validators](/tool/tool-steps/llms/llm-tool-step#validators)
...
### Validation
When there are [output validations](/tool/tool-steps/llms/llm-tool-step#validators) set for an LLM, Relevance automatically checks the output
...
### Temperature
There is a [temperature](/tool/tool-steps/llms/llm-tool-step#temperature) parameter under LLM advance setting. The below error occurs if the entered value is out of the accepted (0,1) range.
```
400: {"message":"-0.5 is less than the minimum of 0 - 'temperature'","type":"invalid_request_error","param":null,"code":null}
```

### History
This error occurs if [History](/tool/tool-steps/llms/llm-tool-step#conversation-history) is set to an empty array. Either enter values, or use the `X` button on the right side of each row to remove the empty rows.
```
