...
at [How to write a good prompt](/build-custom-tools/tool-steps/llms/prompt).
...
* [Conversation history](/build-custom-tools/tool-steps/llms/llm-advance-setting#conversation-history)
* [System prompt](/build-custom-tools/tool-steps/llms/llm-advance-setting#system-prompt)
* [Temperature](/build-custom-tools/tool-steps/llms/llm-advance-setting#temperature)
* [Validators](/build-custom-tools/tool-steps/llms/llm-advance-setting#validators)
* [How to handle large amount of text/context](/build-custom-tools/tool-steps/llms/how-to-handle-too-much-text)
...
![LLM step](/images/build-custom-tools/tool-step/llm-api-key-cost-comparison.png)
...
range; more information is available at [How to handle to much text](/build-custom-tools/tool-steps/llms/how-to-handle-too-much-text).
```
...
### Validation
When there are [output validations](/build-custom-tools/tool-steps/llms/llm-advance-setting#validators) set for an LLM, Relevance automatically checks the output
...
### Too large data
When using Relevance to [handle large inputs by selecting the most relevant entries](/build-custom-tools/tool-steps/llms/how-to-handle-too-much-text), if the 
input data is too large, you need to [upload](/data/data-tables#upload-file) it as a dataset use it as a [knowledge](/build-custom-tools/knowledge/knowledge) 
...
### Temperature
There is a [temperature](/build-custom-tools/tool-steps/llms/llm-advance-setting#temperature) parameter under LLM advance setting. The below error occurs if the entered value is out of the accepted (0,1) range.
```
400: {"message":"-0.5 is less than the minimum of 0 - 'temperature'","type":"invalid_request_error","param":null,"code":null}
```

### History
This error occurs if [History](/build-custom-tools/tool-steps/llms/llm-advance-setting#conversation-history) is set to an empty array. Either enter values, or use the `X` button on the right side of each row to remove the empty rows.
```
Studio transformation prompt_completion input validation error: must be array {"type":"array"} /history
```

### Plan limitations
The below error occurs when GPT-4 is used under a Relevance account with a [plan](/get-started/key-concepts/plans) that does not support the GPT-4 model.
```
