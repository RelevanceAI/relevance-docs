...
On your [API](/tool/api) component, change the body to edit as string and use `{{}}` to access variables.
...
See the full guide at [Adding condition](/tool/tool-steps/getting-started) to a step.
...
### How do I use the [Checkbox input](/tool/user-inputs/check-box) component as a condition for running a step?

[Add a condition](/tool/advanced-setup/conditions) to your step.
...
See the full guide at [Loop through a step](/tool/tool-steps/getting-started#run-a-step-multiple-times-in-a-loop).

### How to reduce hallucination for LLM?

Here are a list of steps to take to improve your experience with LLMs:

- Providing good and precise [system prompt](/tool/tool-steps/llms/llm-advance-setting#system-prompt)
- providing good prompt [Tips on a good prompt](/tool/tool-steps/llms/prompt#tips-on-writing-a-good-prompt-prompt-engineering)
- Providing data as a background [knowledge](/tool/knowledge/knowledge)
...
[How to handle large text](/tool/tool-steps/llms/how-to-handle-too-much-text).
...
[most relevant](/tool/tool-steps/llms/how-to-handle-too-much-text#most-relevant-data).
...
More details are provided at [Output configuration](/tool/advanced-setup/output-configuration)
...
Each output variable in your AI Tool will be added to your data table as a new column.
You can see the results on the knowledge table or export to a CSV.
