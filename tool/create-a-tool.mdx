...
![Empty state of new Tools](/images/build-custom-tools/tool2.png)
1. add knowledge
2. add inputs
3. add tool steps
...
fields; details are fully explained at [knowledge](/build-custom-tools/knowledge/knowledge).
...
details are fully explained at [User inputs](/build-custom-tools/user-inputs/getting-started). 

### Add desired steps
Steps are analysis components. The most frequently used step at Relevance is 
[LLM](/build-custom-tools/tool-steps/llms/llm), which allows you to
...
[Tool steps](/build-custom-tools/tool-steps/getting-started).


## Example
1. On your new Tool page, click on `+ Add input` and then on `+ Text input` 
to add a new input that accepts short pieces of text. 
A [Text input](/build-custom-tools/user-inputs/text-input) component will now appear on the page.
    ...
    * Change the variable name, shown in green, from `text` to `topic`.
    ...
    ![Text inputs](/images/build-custom-tools/user-input/text-input-details1.png)

2. Click on `+ Add step` and then choose [LLM](/build-custom-tools/tool-steps/llms/llm) from the 
...
experiment with text generation relying on LLMs such as GPT. 
    ...
    ![LLM](/images/build-custom-tools/tool-step/llm-get-started.png)
...
4. Run your Tool. You can either go to the `Use` tab or give your tool a try on the current page.
