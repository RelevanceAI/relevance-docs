...
**Add ‘Vector similarity search’ step**

Let’s create our own few-shot prompting technique based on an entire dataset. This can be achieved by using a search step to find the most similar past response to the current input.

![vector search step.png](/images/vector-search-step.png)

1. First we need to select the ‘sales responses’ data set. 

<aside>
💡 In the next part of this series, we'll walk you through the process of creating your own dataset.
...
2. We paste in the name of the vector field. In this case it’s `prospect-reply_openai_vector_` 

<aside>
💡 In the next part of this series, we'll guide you through the process of vectorizing your dataset and identifying the vector field's name.
...
**Add ‘Truncate text’ step**

We aim to feed the results of the ‘Vector similarity search’ into our LLM prompt, so our LLM can generate a response to the sales message based on those shortlisted examples.

To achieve this, we may need to increase the number of results from the vector search. This can be done by increasing the “page size” in the ‘Vector similarity search’ step to, let's say to 30.
...
If we execute the ‘Truncate text’ step, we'll see that it ensures we never have more than 2000 tokens worth of data in the LLM prompt.
