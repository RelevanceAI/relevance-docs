...
Tags provide a flexible, multi-dimensional categorization system for your agents. Unlike folders — where each agent lives in one location — an agent can have multiple tags, letting you slice your agent collection across different dimensions at the same time.

Each tag has a color (gray, red, orange, yellow, green, blue, indigo, purple, or violet) for quick visual identification in the agents list.

#### Creating and managing tags

You need Editor permission to create, update, or delete tags. Tags are scoped to a project and must have unique names within that project. To create a tag, open the tag management panel from your agents list and give the tag a name and color.

#### Applying tags to agents

Open any agent and assign one or more tags from the tag selector. A single agent can carry as many tags as you need — for example, a sales agent could be tagged with both `sales` and `active` and `high-priority`.

#### Permissions

- **Editor**: create, update, and delete tags
- **Viewer**: list and view tags

See the [RBAC documentation](/enterprise/rbac) for a full explanation of permission levels.

#### Tips for effective tagging

- Use tags for cross-cutting concerns that don't map cleanly to a single folder: status (`active`, `draft`, `deprecated`), team ownership (`sales`, `support`, `eng`), or priority (`high-priority`, `review-needed`).
- Keep tag names short and consistent — agree on conventions with your team before creating tags.
- Use folders for hierarchical structure (e.g., by department or project) and tags for attributes that apply across multiple folders.
...
<AccordionGroup>
  ...
  <Accordion title="What are the system limits or quotas for AI agents?">
    Relevance AI has system quotas that define the maximum resource allocations for different aspects of the platform. These may include limits on the number of agents, conversations, knowledge bases, or API calls depending on your plan. Specific details about these quotas can be found in the [system quotas documentation](/admin/system-limits).
  </Accordion>
  ...
  <Accordion title="What is concurrency and how does it affect my agents?">
    Concurrency refers to the number of tasks that can be executed simultaneously by your agents. For more details, visit our [System Quotas page](/admin/system-limits).
  </Accordion>
...
<CardGroup cols={3}>
  <Card
    ...
    icon="plus"
    href="/build/agents/create-an-agent"
  >
    Build your first agent step by step
  </Card>
  <Card
    title="Use Agents in Chat"
    icon="messages"
    href="/get-started/chat/introduction"
  >
  ...
  <Card
    ...
    icon="store"
    href="/get-started/marketplace/introduction"
  >
