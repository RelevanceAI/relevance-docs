...
<Note>
Users with the **Editor** role or above can generate Relevance API keys. On enterprise plans, users with the **Operator** role can also generate their own API key — useful for connecting MCP clients like Claude Desktop or Cursor. Operators can only manage their own keys and cannot manage API keys for other users.
</Note>
