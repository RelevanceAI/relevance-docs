...
<Tabs>
  <Tab title="New billing (Actions and Vendor Credits)">
    ...
    <Warning>
      **Tool retries count as separate Actions.** If a tool is configured to retry on failure, each retry attempt counts as an additional Action. For example, if a tool fails and retries 3 times, that's 4 Actions total (1 initial attempt + 3 retries).
    </Warning>

    ### How Retries Affect Action Usage

    When you configure retry settings for your tools or agents:

    - **Each retry is a separate Action:** Every retry attempt consumes one Action from your plan
    - **Failed attempts still count:** Even if a tool execution fails, it counts as an Action
    - **Accumulation across tools:** If multiple tools in a workflow retry, all retries count toward your total Action usage

    **Example:** If Tool A retries 2 times and Tool B retries 3 times in a single workflow, that's 5 Actions consumed by retries alone, plus the initial executions.

    <Tip>
      Monitor your retry patterns in the Plan & Billing section to identify tools that frequently retry. Addressing underlying issues can help reduce unnecessary Action consumption.
    </Tip>
    ...
    **Actions rollover:** Actions included in your plan will reset to your plan's default amount at each renewal. However, any Action top-ups you purchase will roll over to your next billing cycle. This means purchased Action top-ups carry forward, but your base plan Actions reset.
    ...
    <AccordionGroup>
      ...
      <Accordion title="Do tool retries count as separate Actions?">
        Yes, every tool retry counts as a separate Action. If you configure a tool to retry 3 times on failure, and it fails on the first attempt and succeeds on the second retry, that's 2 Actions total (1 initial + 1 retry).

        This also applies to the Agent Autonomy Limit—each retry counts as a separate tool execution toward the autonomy limit.
      </Accordion>
    </AccordionGroup>
  </Tab>
  <Tab title="Old billing (Credits)">
    ...
    <Warning>
      **Tool retries count as separate credit charges.** If a tool is configured to retry on failure, each retry attempt consumes additional credits. For example, if a tool fails and retries 3 times, you'll be charged for 4 tool executions (1 initial + 3 retries).
    </Warning>
    ...
    <AccordionGroup>
      ...
      <Accordion title="Do tool retries count as separate credit charges?">
        Yes, every tool retry counts as a separate credit charge. If you configure a tool to retry 3 times on failure, and it fails on the first attempt and succeeds on the second retry, you'll be charged for 2 tool executions (1 initial + 1 retry).
