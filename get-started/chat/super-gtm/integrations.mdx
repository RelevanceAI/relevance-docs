...
<AccordionGroup>
  ...
  <Accordion title="Slack">
    **Connection method:** OAuth
    ...
    This is a per-user OAuth connection, separate from the workspace-level bot token used by [Relevance AI's Builder features (Agents, Tools, Triggers, Workforces)](/integrations/popular-integrations/slack). Workspace admins can allow one connection type while blocking the other — if Slack works in Builder but not here (or vice versa), check with your Slack workspace admin about per-user OAuth app permissions.
