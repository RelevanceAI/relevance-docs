...
<Info>
Subagents replaces the deprecated agents-as-skills approach. Previously, Super GTM would "become" a connected Agent by reading its instructions and running its tools inline. Subagents instead run each connected Agent in its own conversation with its own context.
</Info>
