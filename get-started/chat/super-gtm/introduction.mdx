...
The meetings widget in the Super GTM interface shows a unified calendar view from both Google Calendar and Microsoft Calendar (Outlook/Teams). Connect one or both providers and all your upcoming meetings appear together in a single view, regardless of which calendar they came from.

Each meeting entry shows the subject, location, attendees, and any Teams meeting links. Meeting prep or summaries that Super GTM generates are automatically matched to their corresponding meeting entry, whether the event originated from Google Calendar or Microsoft Calendar.

To populate the meetings widget, connect Google Calendar (via the GSuite integration) or Microsoft Calendar from your [integration settings](/get-started/chat/super-gtm/integrations).
