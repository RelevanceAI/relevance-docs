...
You can view and manage your skills in the **Skills** tab in the right-side menu.

<Frame>
  <img src="/images/where-to-find-your-skills.png" alt="Skills tab in the Super GTM right-side menu" />
</Frame>

From here you can browse all available skills (project, user, and built-in), see which skills are active, and open any skill to view or edit its contents.
...
<Tabs>
  ...
  <Tab title="User skills">
    ...
    You can create and edit your own user skills. Other team members can discover and read them, but can't modify them.
  </Tab>
  <Tab title="Built-in skills">
    Skills that ship with the platform. These include:

    - **Skill Creator** — helps you build new skills (see [How to create a skill](#how-to-create-a-skill) below)
    - **Meeting Prep & Summary** — prepares you for upcoming meetings using your calendar, transcripts, and notes (assumes you have a meeting integration setup like Google Calendar — use this as a starting example)

    Built-in skills are read-only and available to everyone.
  </Tab>
  <Tab title="Agent-as-skills">
    Other agents in your project that have been made available to Super GTM. Unlike regular skills (which are instructions Super GTM follows), agent-as-skills have their own **tools** and run independently when activated.

    To enable an agent as a skill, open it in the agent builder, go to **Advanced**, and toggle on **Can be used by Super GTM**. It will then appear in the skills catalog.
  </Tab>
</Tabs>
...
<Accordion title="Building the skill from a conversation">
  ...
  > "Summarize my call with Moonlight Labs. Pull the action items, update the deal in HubSpot, and draft a follow-up email to the attendees."
...
<Accordion title="Using the skill afterward">
  ...
  - "Follow up on my call with Moonlight Labs"
...
Skills don't have to be triggered by a conversation. You can schedule the agent to run a skill automatically on a recurring basis — daily, weekly, or at whatever cadence you need. Set up schedules in the **Overview** tab in the right-side panel.
...
To set up a schedule, go to the **Schedules** section in the UI. Choose the skill, set the cadence, and the agent runs it automatically.

## Tips for writing good skills

Keep these in mind when creating skills:

<AccordionGroup>
  <Accordion title="Describe when it should activate">
    Include the kinds of things you'd say when you need this workflow. The agent uses this to match your requests to the right skill.
  </Accordion>
