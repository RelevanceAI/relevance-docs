...
Enterprise customers can enable PII redaction to automatically detect and protect Personally Identifiable Information (PII) in their OTEL logs. This feature uses AI-powered entity recognition to identify and redact sensitive data before it's delivered to your S3 bucket.
...
| `entities` | string[] | No | List of PII entity types to detect and redact. If omitted, all supported entities are used |
| `score_threshold` | float | No | Confidence threshold for entity detection (0.0-1.0). Higher values require more confidence |

### Supported PII Entities

The following entity types are detected by default:
