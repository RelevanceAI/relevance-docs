...
details are fully explained at [User inputs](/custom-tools/user-inputs/getting_started). 

### Add desired steps
Steps are analysis components. The most frequently used step at Relevance is 
[LLM](/custom-tools/tool-steps/llms/llm), which allows you to
...
[Tool steps](/custom-tools/tool-steps/getting_started).
...
A [Text input](/custom-tools/user-inputs/text-input) component will now appear on the page.
    ...
    * Change the variable name, shown in green, from `text` to `topic`.
    ...
    ![Text inputs](/images/tools/components/user-input/text-input-details1.png)

2. Click on `+ Add step` and then choose [LLM](/custom-tools/tool-steps/llms/llm) from the 
...
experiment with text generation relying on LLMs such as GPT. 
    ...
    ![LLM](/images/tools/components/tool-step/llm-get-started.png)
