...
* **Provide the transcript**: Use copy-paste to provide the transcript into the Tool. Or when in a [bulk-run](/templates/bulk-run), select 
the corresponding column containing the transcription.

<Tip>
When an audio file is [uploaded](/data/data-tables#upload-file) to Relevance AI, the platform automatically transcribes it. Results are 
