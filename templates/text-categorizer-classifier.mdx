...
![Categorize text](/images/templates/text-categorizer-classifier/text-categorizer-classifier-form.png)
...
**NOTE**: Examples are to cover your coding style not to teach the LLM about your specific data related categorization.
For instance, if in a dataset, anything related to a dorm (tidiness, cleanness, noise, etc.) is considered "Amenities",
since "Amenities" is not a near synonym for any of those categories, providing examples will not necessarily result LLMs to 
categorize "I wish they cleaned the kitchen more frequently" under "Amenities".

<Tip>
**Good Categories**

LLMs work is to consider 
Taken from your examples:
"Cheaper" is semantically close to Affordable (in other words the word affordable is a near-synonym)
"Make my property value go down" is a good map to "Property value decline" (another near-synonym)
But
"Domestic violence" does not bring "less fortunate" or "disadvantaged" to mind.
Messy / Structure / Maintenance / etc. are not near-synonyms for Amenity. The relation to Amenity is the context that this specific dataset specifies and a 
general-purpose LLM is not expected to know it. We do not want to force the model to work with word matching or rule-based results as moving away from 
semantics is like moving backwards.

What I recommend is to 
keep these (Messy / Structure / Maintenance / etc.) as separate categories
categorize the data and export the results into a CSV file
merge all child categories to Amenities as the parent category, in a software like Excel
...
3. Meaningful combination: Use `/` to combine two relevant categories (e.g. Customer Service / Support)
