...
<Note>
  Email is the unique identifier for each Relevance AI account. If a user's email address changes in your identity provider, they will receive a new Relevance AI account rather than having their existing account updated—permissions, group memberships, and history from the original account do not transfer automatically. See [Email address updates for IdP-managed users](/enterprise/rbac-groups#email-address-updates-for-idp-managed-users) for the recommended workaround.
</Note>
