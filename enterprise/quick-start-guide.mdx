...
<Columns cols={2}>
  ...
  <Card title="Project feature controls" icon="toggle-on">
    Control which platform features — such as Invent — are available to team members within each project
  </Card>
</Columns>
...
<Steps>
  ...
  <Step title="Configure project feature controls (optional)" icon="toggle-on">
    **Why this matters**: Project feature controls let you decide which platform features — starting with Invent — are available to team members in each project. Features default to disabled, so you control rollout project by project.

    **What you'll do**:
    - Navigate to **Settings → Organization → Projects → Manage settings → Features** for each project
    - Enable Invent for projects where you want team members to access it
    - Confirm that organization admins and owners retain access regardless of the setting

    **Time required**: 5 minutes per project

    [View project feature controls guide →](/admin/project-management/project-features)
  </Step>
...
| Project feature controls (Invent toggle) | ✅ Available | Now |
