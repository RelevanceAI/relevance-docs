...
<AccordionGroup>
  ...
  <Accordion title="Email address updates for SSO/SCIM users">
...
<AccordionGroup>
  ...
  <Accordion title="What happens if a user's email address changes in our identity provider?">
    Email address updates are not currently supported for SSO/SCIM managed users. Changing a user's email in your IdP provisions a new Relevance AI account — the original account's permissions, group memberships, and owned assets are not transferred. See [Email address updates for IdP-managed users](#email-address-updates-for-idp-managed-users) for the step-by-step workaround.
  </Accordion>
</AccordionGroup>
