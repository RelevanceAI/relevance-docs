...
**`workforce_task_deleted`** - Emitted when a workforce task (a single execution run of a workforce) is deleted. One event is emitted per deleted task ID, including during bulk deletes.

| Attribute | Type | Required | Description |
| --- | --- | --- | --- |
| `relevance_ai.event.workforce_task_id` | string | Yes | ID of the deleted workforce task. Matches the `relevance_ai.workforce.workforce_task_id` attribute on workforce execution spans |
