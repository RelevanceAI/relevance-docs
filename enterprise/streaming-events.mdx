...
<CardGroup cols={3}>
  <Card title="EMAIL_ADDRESS" icon="envelope" />
  <Card title="PHONE_NUMBER" icon="phone" />
  <Card title="PERSON" icon="user" />
  <Card title="CREDIT_CARD" icon="credit-card" />
  <Card title="US_SSN" icon="id-card" />
  <Card title="IP_ADDRESS" icon="globe" />
  <Card title="IBAN_CODE" icon="building-columns" />
  <Card title="URL" icon="link" />
  <Card title="DATE_TIME" icon="calendar" />
...
| `relevance_ai.request_id` | string | No | The request ID that produced this record. Matches the `x-request-id` response header returned by the Relevance AI API, enabling correlation between API responses and the OTEL records they generated. |
    ...
    "scopeLogs": [{
      "logRecords": [{
        ...
        "attributes": [
          ...
          { "key": "relevance_ai.request_id", "value": { "stringValue": "01966e4e-1234-7abc-def0-1234567890ab" }},
...
## Correlating API responses to OTEL records

Both log and trace records include a `relevance_ai.request_id` attribute that matches the `x-request-id` response header returned by the Relevance AI API. Capture this header from your API responses to filter your S3 or Databricks exports and find the exact records produced by a specific call.

Workflows this enables:

- **Debugging**: isolate all OTEL records for a specific invocation when investigating unexpected behavior
- **Correlation**: link your application logs to OTEL records when both reference the same request ID
- **Compliance audits**: retrieve a complete record of activity tied to a known request
- **Troubleshooting**: respond to user-reported issues by pulling the exact execution trace

<Accordion title="Filtering by request ID using AWS Athena">

If you store your OTEL exports in S3 and query them with Athena, use `UNNEST` to expand the attributes array and filter by `relevance_ai.request_id`:

```sql
SELECT *
FROM your_otel_logs_table
CROSS JOIN UNNEST(attributes) AS t(attr)
WHERE attr.key = 'relevance_ai.request_id'
  AND attr.value.stringvalue = '01966e4e-1234-7abc-def0-1234567890ab'
```

Run the same query against your traces table to retrieve the corresponding spans. The `x-request-id` header is present on every Relevance AI API response — store it alongside any user-facing identifiers in your application logs to enable end-to-end tracing from your application through to the OTEL export.
