...
Queued tasks only appear once your organization hits its concurrent task limit. That limit is shown in the top-right corner of the chart as **Max concurrency**. If you regularly see a red queued band, the limit is likely a bottleneck — raising it means moving to a higher plan or arranging a custom limit.

### Per-project breakdown

There is no view switcher on this chart — what you see depends on your permissions. If you can manage the organization, the bars are split by project: each project gets its own running and queued bands, ordered so the highest-demand project is the darkest shade. Everyone else sees the whole organization as a single band labeled **Concurrency**.

When the breakdown is shown, hovering over any bar opens a tooltip with **Running** and **Queued** columns, one row per project, and a **Total** row at the bottom — so you can identify which projects contribute most to concurrency load at any given time, alongside the organization's concurrency limit.

<Note>
  The **Org-wide** / **This project** toggle sits in the Live jobs table below the chart and filters that table only. It does not change what the chart shows.
</Note>
