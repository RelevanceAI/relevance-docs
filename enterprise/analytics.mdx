...
## Agent-level concurrency monitoring

Each agent's analytics page includes a concurrency usage chart that shows organization-wide concurrency data. This view lets you correlate a specific agent's activity with overall concurrency patterns, helping you identify which agents are most active during peak concurrency periods.

To view concurrency for a specific agent, navigate to the agent, then click the **Analytics** tab.

<Note>
  For information about concurrency limits by subscription tier and how to monitor usage at the organization level, see [Plans and credits](/admin/subscriptions/plans#monitoring-concurrency-usage).
</Note>
