...
The chart displays both running and queued tasks by default. When queued task data exists in the selected time range, a **Show queued** toggle appears on the chart. Hiding queued bars rescales the y-axis, making running concurrency easier to read when a large queue dominates the chart scale. Bars with hidden queued work show a footnote in their tooltip — for example, "+4,000 queued (hidden)" — so the queued volume remains visible. The toggle preference persists across page reloads.
