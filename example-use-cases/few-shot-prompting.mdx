...
To start, [create](/build/tools/create-a-tool) a new tool. 
Add a [long text](/build/tools/user-inputs/long-text-input) input, to present the message to which we want to respond. Next, add an
[LLM](/build/tools/tool-steps/llms/llm) component.
...
past response(s) to the current input. So, we need to add a [Knowledge search](/build/tools/tool-steps/knowledge-search) component to our Tool and
configure the step:
...
We need to add a [truncate text](/build/tools/tool-steps/truncate-text) step to truncate the results. This allows us to specify an array, which is derived from the result 
