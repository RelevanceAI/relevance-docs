...
When using the Zendesk API Call tool step, the authentication headers are automatically included. You only need to specify:
...
<AccordionGroup>
  ...
  <Accordion title="Do I need an Enterprise plan to use Zendesk with Relevance AI?" icon="crown">
    Zendesk tool steps (actions) are available on all Relevance AI plans. However, **Zendesk triggers** (automatic activation when events occur in Zendesk) are only available on Enterprise plans. Contact our sales team to learn more about Enterprise features.
  </Accordion>
  ...
  <Accordion title="What's the difference between pre-built tool steps and the API Call tool step?" icon="code-compare">
    Pre-built tool steps provide a simplified interface for common Zendesk actions with guided inputs. The API Call tool step gives you direct access to the full Zendesk API for advanced use cases and custom workflows that aren't covered by pre-built steps.
  </Accordion>
