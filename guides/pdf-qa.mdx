...
```bash
$ npm install -g @relevanceai/chains
```
...
https://colab.research.google.com/drive/10hHATBigT1vB5dJTjrS8dAjc0pId0lKp?usp=sharing
...
### Loading our PDFs into our built-in vector database
...
We'll be creating more tooling and content around this process, but for now you can check out a simple [Google Colab showing how](https://colab.research.google.com/drive/10hHATBigT1vB5dJTjrS8dAjc0pId0lKp?usp=sharing). To run, you'll have to fill in your authentication details at the top of the notebook.
...
We start by importing the `defineChain` method from the `@relevanceai/chains` package:

```js
import { defineChain } from '@relevanceai/chains';
...
Note: the `setup` function only allows you to declare each step of a chain. No Javascript you write outside of this paradigm, except for your returned output will be executed.

However, you can use the `code` helper function provided in `setup` to run custom Javascript. You can also use the `runIf` function to conditionally run steps. [Learn more.]('/reference/setup')
...
const chain = defineChain({
    ...
    setup({ params, step, code, runIf }) {
        ...
        const { modifiedQuestion, reducedHistory } = runIf(chatHistory, () => {
          ...
          return { modifiedQuestion, reducedHistory };
        });

        const { transformed: questionToUse } = code(({ history, modifiedQuestion, question }) => ({ history, modifiedQuestion, question }) => {
          return history && history.length ? modifiedQuestion : question;
        });
        ...
        const { transformed: resultsMapped } = code(({ vectorSearchResults }), ({ vectorSearchResults }) => vectorSearchResults.map((result) => {
          text: r.text,
          url: r.pdf_url
        }));
...
The key code exists in the `useChain` composable. We run the chain by it's ID.
