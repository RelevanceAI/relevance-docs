...
description: 'In this guide I\'ll show you how to create a ChatGPT-like question & answer widget for your PDFs.'
---
...
We have a built in vector database, but for this tutorial let's have a look at using a third party vector database such as Redis.
...
Note: the `setup` function only allows you to declare each step of a chain. No Javascript you write outside of this paradigm, except for your returned output will be executed. You can use `js_code_transformation` to execute Javascript, as well as the `code` helper function provided in `setup` to convert your function into a variable.
...
const chain = defineChain({
    ...
    setup({ params, step, code }) {
        ...
        // Step 1. let's turn the chat history into a string for prompt
        const { transformed: reducedHistory } = step('js_code_transformation', {
            code: `return ${historyReducer}`,
        });
...
Standalone question:`,
        ...
        const resultsMapper = code(vectorSearchResults.map((result) => {
          text: r.text,
          url: r.pdf_url
        }));

        // let's map the results to be a little cleaner
        const { transformed: resultsMapped } = step('js_code_transformation', {
          code: `return ${resultsMapper}`
        });
...
Helpful Answer JSON:`
        ...
        const answerConverter = code(function () {
        ...
        const { transformed: answerJSON } = step('js_code_transformation', {
          code: `return ${answerConverter}`
        });
