...
<Tab title="No build" id="no-build">
  ...
  You don't need to touch the builder. Run the cloned Agent above as-is, or skip cloning entirely and have a conversation with SuperGTM.

  ## Talk to SuperGTM

  SuperGTM is a chat-mode super agent that works across your entire sales stack — CRM, calendar, email, calls, support tickets — without you picking or configuring anything per Agent.

  You ask in plain English. It pulls data from the right tools, takes actions on your behalf (with your approval before anything writes), and invokes other Agents in your workspace when needed.

  <Card title="Set up SuperGTM" icon="wand-magic-sparkles" href="/get-started/chat/super-gtm/introduction">
...
<Tab title="Build with AI" id="build-with-ai">
  ...
  Anything you build — including Agents shaped through Programmatic GTM — can run inside **[SuperGTM](/get-started/chat/super-gtm/introduction)** in chat mode. Build the Agent the way that suits you, then your sales team can invoke it conversationally without leaving chat.
</Tab>
</Tabs>
</div>
