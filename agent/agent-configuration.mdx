...
We will soon offer markdown support for any area that accepts LLM prompts, like the core instructions, the flow builder and the LLM Tool/Tool Steps.
...
| Heading 1-6       | `Cmd+1` to `Cmd+6`     | `# H1`... `###### H6`             |
| Blockquote        | `Cmd+Shift+Q`          | `> blockquote`                                              |
| Bullet Lists      | `Cmd+Shift+8`          | `- List item start with a dash`, or a `*` or `+`. Nest sub-items with 2 spaces or tabs.                   |
