...
Agents in the same project can share tools with each other, making it easy to reuse tools across multiple agents without duplication.

**How it works:**
- When you add a tool to one agent, other agents in your project can access and use that same tool
- Shared tools maintain consistent configurations across all agents that use them
- Changes to a shared tool are reflected for all agents using it

**Benefits of tool sharing:**
- **Consistency**: Ensure all agents use the same version of a tool with identical configurations
- **Efficiency**: Build tools once and reuse them across multiple agents
- **Maintenance**: Update a tool in one place and have changes apply to all agents using it

**To share a tool with another agent:**
1. Navigate to the Tools section in your Agent Builder
2. Select "Add new tool" or choose from existing tools in your project
3. Tools created by any agent in your project will be available for selection
4. Configure the tool's usage settings for this specific agent

<Note>Tool sharing is available for agents within the same project. Tools shared between agents in a Workforce can also be configured through the [Workforce Builder](/docs/workforce/build-an-ai-workforce/add-tools).</Note>
