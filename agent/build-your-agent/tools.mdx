...
The tool sharing feature makes it easy to reuse tools across multiple agents in your project. When you add a tool to one agent, you can quickly share it with other agents without having to reconfigure settings each time.

### Benefits of Tool Sharing

<CardGroup cols={2}>
  <Card title="Consistency" icon="check-double">
    Ensure all agents use the same tool configuration, reducing errors and maintaining consistency across your project.
  </Card>
  <Card title="Efficiency" icon="bolt">
    Save time by configuring a tool once and sharing it with multiple agents instead of setting it up repeatedly.
  </Card>
  <Card title="Easy Updates" icon="arrows-rotate">
    Update a shared tool in one place and have those changes reflected across all agents using it.
  </Card>
  <Card title="Collaboration" icon="users">
    Team members can easily discover and reuse tools that others have created and configured.
  </Card>
</CardGroup>

### How to Share Tools

When adding tools to your agent, you can:

1. **Browse shared tools**: View tools that are already configured and available in your project
2. **Add to multiple agents**: Select a tool and add it to any agent that needs that capability
3. **Maintain configurations**: Tool settings are preserved when shared, ensuring consistent behavior

<Tip>
Use descriptive names and clear descriptions for your tools to make them easier to find and understand when sharing across agents.
</Tip>
...
- **Leverage Shared Tools**: Before creating a new tool, check if a suitable shared tool already exists in your project
